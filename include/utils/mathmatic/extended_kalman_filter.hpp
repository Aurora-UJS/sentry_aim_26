...
template <typename T>
concept MatrixType = std::same_as<T, Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic>>;
...
public:
    ...
    [[nodiscard]] double getResidualNorm() const noexcept { return last_residual_.norm(); }
    ...
    [[nodiscard]] MatrixX1 update(const MatrixZ1& z) noexcept {
        ...
        for ([[maybe_unused]] const auto iter : std::views::iota(0, iteration_num_)) {
            ...
            MatrixZZ S = H * P_pri * H.transpose() + R + SmallNoise * MatrixZZ::Identity();
