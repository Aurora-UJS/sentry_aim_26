...
template <typename T>
concept MatrixType =
    std::same_as<T, Eigen::Matrix<double, Eigen::Dynamic,
...
concept PredictFunctor =
    std::invocable<F, StateType> &&
    std::same_as<std::invoke_result_t<F, StateType>, StateType>;

template <typename F, typename StateType, typename MeasureType>
concept MeasureFunctor =
    std::invocable<F, StateType> &&
    std::same_as<std::invoke_result_t<F, StateType>, MeasureType>;
...
public:
  ...
  [[nodiscard]] MatrixX1 update(const MatrixZ1 &z) noexcept {
    ...
    for ([[maybe_unused]] const auto iter :
      ...
      for (const auto i : std::views::iota(0, SigmaPoints)) {
        ...
        for (const int idx : angle_dims_) {
          dz[idx] =
              angles::shortest_angular_distance(z_pred[idx], Zsig.col(i)[idx]);
        }
    ...
    for (const auto i : std::views::iota(0, SigmaPoints)) {
      ...
      for (const int idx : angle_dims_) {
        dz[idx] = angles::shortest_angular_distance(z_pred_final[idx],
                                                    Zsig_final.col(i)[idx]);
      }
...
private:
  ...
  Eigen::Matrix<double, N_X, SigmaPoints>
      Xsig_pred; ///< Predicted sigma points matrix
  ...
  void generateSigmaPoints(
      const MatrixX1 &x, const MatrixXX &P,
      Eigen::Matrix<double, N_X, SigmaPoints> &Xsig_out) noexcept {
    const auto A =
        P.llt().matrixL(); // Cholesky decomposition (optimized in Eigen)
    Xsig_out.col(0) = x;
