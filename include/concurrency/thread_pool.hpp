...
public:
  ...
  template <typename F> MoveOnlyFunction(F &&f) {
...
private:
  ...
  template <typename F> struct Impl : Concept {
...
public:
  ...
  ~OrderedQueue() {
    ...
    buffer_.clear();
    current_id_ = 0;
    std::cerr << "OrderedQueue destroyed, queue and buffer cleared."
              << std::endl;
  }
  ...
  bool enqueue(const Frame &item) {
    if (drop_pred_(item)) {
      std::cerr << "OrderedQueue: Dropped frame with id: " << item.id
                << std::endl;
      return false;
    }
    std::lock_guard<std::mutex> lock(mutex_);
    if (main_queue_.size() + buffer_.size() >= max_size_) {
      std::cerr << "OrderedQueue: Dropped frame due to full queue: " << item.id
                << std::endl;
      throw std::runtime_error("OrderedQueue is full");
    }
    if (item.id < current_id_) {
      std::cerr << "OrderedQueue: Dropped frame with small id: " << item.id
                << std::endl;
      return false;
    }
    if (item.id == current_id_) {
...
public:
  ...
  explicit ThreadPool(
      ...
      : m_high_priority_queue_(queue_capacity),
        m_normal_priority_queue_(queue_capacity), m_stop_(false),
  ...
  auto submit_frame(const Frame &frame, F &&processor)
    ...
    enqueue_task(
        [this, promise_ptr, id = frame.id,
         proc = std::forward<F>(processor)]() mutable {
          try {
  ...
  size_t pending_tasks() const {
    return m_pending_tasks_.load(std::memory_order_relaxed);
  }
...
private:
  ...
  void worker_loop() {
    while (true) {
      ...
      if (m_high_priority_queue_.pop(task_ptr) ||
          m_normal_priority_queue_.pop(task_ptr)) {
        std::unique_ptr<Task> guard(task_ptr);
        try {
          (*task_ptr)();
        } catch (const std::exception &e) {
          std::cerr << "ThreadPool: Task threw an exception: " << e.what()
                    << std::endl;
        } catch (...) {
  ...
  void enqueue_task(Task &&task, bool high_priority) {
    ...
    auto &queue =
        high_priority ? m_high_priority_queue_ : m_normal_priority_queue_;
  ...
  boost::lockfree::queue<Task *>
      m_high_priority_queue_; // High-priority task queue
  boost::lockfree::queue<Task *>
      m_normal_priority_queue_; // Normal-priority task queue
  std::counting_semaphore<> m_tasks_available_sem_{
      0}; // Semaphore for task availability
