 ...
 *
 * Training C and C++ APIs are an extension of the \ref c_cpp_api "onnxruntime core C and C++ APIs"
 ...
 *
 * These training artifacts can be generated as part of an offline step using the python
 * [utilities](https://github.com/microsoft/onnxruntime/blob/main/orttraining/orttraining/python/training/onnxblock/README.md)
 * made available in the `onnxruntime-training` python package.
 *
 * After these artifacts have been generated, the C and C++ utilities listed in this documentation
 * can be leveraged to perform training.
 *
 * If any problem is encountered, please create an
 * [issue](https://github.com/microsoft/onnxruntime/issues/new) with your scenario and requirements,
 * and we will be sure to respond and follow up on the request.
 *
 ...
 *
 * > **Note**
 * > The ::OrtCheckpointState contains the entire training state that the ::OrtTrainingSession uses.
 * As a result, the training session must always have access to the state. That is to say, the
 * ::OrtCheckpointState instance must outlive the lifetime of the ::OrtTrainingSession instance.
 *
 * <h1>Training C++ API</h1>
 *
 ...
 * ```
 * > **Note**
 * > The ::Ort::CheckpointState contains the entire training state that the ::Ort::TrainingSession
 * uses. As a result, the training session must always have access to the state. That is to say, the
 * ::Ort::CheckpointState instance must outlive the lifetime of the ::Ort::TrainingSession instance.
 */
 ...
 */
ORT_RUNTIME_CLASS(
    TrainingSession);  // Type that enables performing training for the given user models.
ORT_RUNTIME_CLASS(
    CheckpointState);  // Type that holds the training states for the training session.
...
struct OrtTrainingApi {
     ...
     * \note Note that the training session created with a checkpoint state uses this state to store
     * the entire training state (including model parameters, its gradients, the optimizer states
     * and the properties). As a result, it is required that the checkpoint state outlive the
     * lifetime of the training session.
     * \note Note that the checkpoint file can be either the complete checkpoint or the nominal
     * checkpoint.
     *
     * \param[in] checkpoint_path Path to the checkpoint file
     * \param[out] checkpoint_state Checkpoint state that contains the states of the training
     * session.
     *
     ...
     * \param[in] checkpoint_path Path to the checkpoint file.
     * \param[in] include_optimizer_state Flag to indicate whether to save the optimizer state or
     * not.
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     *
     */
    ORT_API2_STATUS(SaveCheckpoint, _In_ OrtCheckpointState* checkpoint_state,
     ...
     *
     * This function creates a training session based on the env and session options provided that
     ...
     * device specified by the user through the session options (if necessary). The training session
     * requires four training artifacts
     * - The training onnx model
     * - The evaluation onnx model (optional)
     * - The optimizer onnx model
     * - The checkpoint file
     *
     * These artifacts can be generated using the `onnxruntime-training` python
     * [utility](https://github.com/microsoft/onnxruntime/blob/main/orttraining/orttraining/python/training/onnxblock/README.md).
     *
     ...
     * \param[in] checkpoint_state Training states that the training session uses as a starting
     * point for training.
     ...
     *
     */
    ORT_API2_STATUS(CreateTrainingSession, _In_ const OrtEnv* env,
     ...
     * \param[in] optim_model_data Buffer containing the model data to be used to perform weight
     * update
    ...
    ORT_API2_STATUS(CreateTrainingSessionFromBuffer, _In_ const OrtEnv* env,
                    _In_ const OrtSessionOptions* options,
                    _Inout_ OrtCheckpointState* checkpoint_state, _In_ const void* train_model_data,
                    size_t train_data_length, _In_ const void* eval_model_data,
                    size_t eval_data_length, _In_ const void* optim_model_data,
                    size_t optim_data_length, _Outptr_result_maybenull_ OrtTrainingSession** out);
     ...
     *
     */
    ORT_API2_STATUS(TrainingSessionGetTrainingModelOutputCount, _In_ const OrtTrainingSession* sess,
                    _Out_ size_t* out);
     ...
     *
     */
    ORT_API2_STATUS(TrainingSessionGetEvalModelOutputCount, _In_ const OrtTrainingSession* sess,
                    _Out_ size_t* out);

    /** \brief Retrieves the names of user outputs in the training model.
     *
     * This function returns the names of outputs of the training model that can be associated with
     * the OrtValue(s) returned by the OrtTrainingApi::TrainStep function.
     *
     * \param[in] sess The `this` pointer to the training session.
     ...
     *
     */
    ORT_API2_STATUS(TrainingSessionGetTrainingModelOutputName, _In_ const OrtTrainingSession* sess,
                    size_t index, _Inout_ OrtAllocator* allocator, _Outptr_ char** output);

    /** \brief Retrieves the names of user outputs in the eval model.
     *
     * This function returns the names of outputs of the eval model that can be associated with the
     * OrtValue(s) returned by the OrtTrainingApi::EvalStep function.
     *
     * \param[in] sess The `this` pointer to the training session.
     * \param[in] index Index of the output name requested.
     * \param[in] allocator Allocator to use to allocate the memory for the name.
     ...
     *
     */
    ORT_API2_STATUS(TrainingSessionGetEvalModelOutputName, _In_ const OrtTrainingSession* sess,
                    size_t index, _Inout_ OrtAllocator* allocator, _Outptr_ char** output);
     ...
     *
     * This function sets the internal state of the training session such that the gradients of the
     * trainable parameters in the OrtCheckpointState will be scheduled to be reset just before the
     * new gradients are computed on the next invocation of the next OrtTrainingApi::TrainStep.
     *
    ...
    /** \brief Computes the outputs of the training model and the gradients of the trainable
     * parameters for the given inputs
     *
     * This function performs a training step that computes the outputs of the training model and
     * the gradients of the trainable parameters for the given inputs. The train step is performed
     * based on the training model that was provided to the training session. The
     * OrtTrainingApi::TrainStep is equivalent of running forward propagation and backward
     * propagation in a single step. The gradients computed are stored inside the training session
     * state so they can be later consumed by the OrtTrainingApi::OptimizerStep function. The
     * gradients can be lazily reset by invoking the OrtTrainingApi::LazyResetGrad function.
     *
     * \param[in] sess The `this` pointer to the training session.
     ...
     *
     */
    ORT_API2_STATUS(TrainStep, _Inout_ OrtTrainingSession* sess,
     ...
     *
     * This function performs an eval step that computes the outputs of the eval model for the given
     * inputs. The eval step is performed based on the eval model that was provided to the training
     * session.
     *
     * \param[in] sess The `this` pointer to the training session.
     ...
     *
     */
    ORT_API2_STATUS(EvalStep, _In_ const OrtTrainingSession* sess,
                    _In_opt_ const OrtRunOptions* run_options, _In_ size_t inputs_len,
                    _In_reads_(inputs_len) const OrtValue* const* inputs, _In_ size_t outputs_len,
                    _Inout_updates_all_(outputs_len) OrtValue** outputs);
     ...
     * \note Please note that this function does not set the initial learning rate that may be
     * needed by the predefined learning rate schedulers. To set the initial learning rate for
     * learning rate schedulers, please look at the function
     * OrtTrainingApi::RegisterLinearLRScheduler.
     *
     * \param[in] sess The `this` pointer to the training session.
     ...
     *
     * This function performs the weight update step that updates the trainable parameters such that
     * they take a step in the direction of their gradients (gradient descent). The optimizer step
     * is performed based on the optimizer model that was provided to the training session. The
     * updated parameters are stored inside the training state so that they can be used by the next
     ...
     * multiplicative factor from the initial learning rate set on the training session to 0. The
     * decay is performed after the initial warm up phase where the learning rate is linearly
     * incremented from 0 to the initial learning rate provided.
     *
     * \param[in] sess The `this` pointer to the training session.
     ...
     *
     */
    ORT_API2_STATUS(RegisterLinearLRScheduler, _Inout_ OrtTrainingSession* sess,
                    _In_ const int64_t warmup_step_count, _In_ const int64_t total_step_count,
                    _In_ const float initial_lr);

    /** \brief Update the learning rate based on the registered learing rate scheduler.
     *
     * Takes a scheduler step that updates the learning rate that is being used by the training
     ...
     * session.
     * \note Please note that a valid predefined learning rate scheduler must be first registered to
     ...
     *
     * Calculates the total number of primitive (datatype of the parameters) elements of all the
     * parameters in the training state. When trainable_only argument is true, the size is
     * calculated for trainable params only.
     *
     * \param[in] sess The `this` pointer to the training session.
     ...
     *
     */
    ORT_API2_STATUS(GetParametersSize, _Inout_ OrtTrainingSession* sess, _Out_ size_t* out,
                    bool trainable_only);
     ...
     * with matching setting for the argument trainable_only. All the target parameters must be of
     * the same datatype. The OrtValue must be pre-allocated onto the desired device. This is a
     * complementary function to OrtTrainingApi::CopyBufferToParameters. Parameter ordering is
     * preserved. User is responsible for allocating and freeing the resources used by the
     * parameters_buffer.
     *
     * \param[in] sess The `this` pointer to the training session.
     * \param[in] trainable_only Whether to skip non-trainable parameters
    ...
    /** \brief Copy parameter values from the given contiguous buffer held by parameters_buffer to
     * the training state
     *
     * The parameters_buffer argument has to be of the size given by
     * OrtTrainingApi::GetParametersSize api call, with matching setting for trainable_only
     * argument. All the target parameters must be of the same datatype. This is a complementary
     * function to OrtTrainingApi::CopyParametersToBuffer and can be used to load updated buffer
     * values onto the training state. Parameter ordering is preserved. User is responsible for
     * allocating and freeing the resources used by the parameters_buffer. In case the training
     * session was created with a nominal checkpoint, invoking this function is required to load the
     * updated parameters onto the checkpoint to complete it.
     *
     * \param[in] sess The `this` pointer to the training session.
     * \param[in] trainable_only Whether to skip non-trainable parameters
     ...
     * \note Note that the checkpoint state must be released only after the training session has
     * been released.
     *
     */
     ...
     * are used to prune the eval model so that the inference model's outputs align with the
     * provided outputs. The exported model is saved at the path provided and can be used for
     * inferencing with InferenceSession.
     * \note Note that the function re-loads the eval model from the path provided to
     * OrtTrainingApi::CreateTrainingSession and expects that this path still be valid.
     *
     * \param[in] sess The `this` pointer to the training session.
     ...
     *
     * This function returns the number of inputs of the training model so that the user can
     * accordingly allocate the OrtValue(s) provided to the OrtTrainingApi::TrainStep function.
     *
     * \param[in] sess The `this` pointer to the training session.
     ...
     *
     */
    ORT_API2_STATUS(TrainingSessionGetTrainingModelInputCount, _In_ const OrtTrainingSession* sess,
                    _Out_ size_t* out);
     ...
     *
     */
    ORT_API2_STATUS(TrainingSessionGetEvalModelInputCount, _In_ const OrtTrainingSession* sess,
                    _Out_ size_t* out);

    /** \brief Retrieves the name of the user input at given index in the training model.
     *
     * This function returns the names of inputs of the training model that can be associated with
     * the OrtValue(s) provided to the OrtTrainingApi::TrainStep function.
     *
     * \param[in] sess The `this` pointer to the training session.
     ...
     *
     */
    ORT_API2_STATUS(TrainingSessionGetTrainingModelInputName, _In_ const OrtTrainingSession* sess,
     ...
     *
     * This function returns the names of inputs of the eval model that can be associated with the
     * OrtValue(s) provided to the OrtTrainingApi::EvalStep function.
     *
     * \param[in] sess The `this` pointer to the training session.
     ...
     *
     */
    ORT_API2_STATUS(TrainingSessionGetEvalModelInputName, _In_ const OrtTrainingSession* sess,
                    size_t index, _In_ OrtAllocator* allocator, _Outptr_ char** output);
     ...
     *
     * Runtime properties such as epoch, training step, best score, and others can be added to the
     * checkpoint state by the user by calling this function with the corresponding property name
     * and value. The given property name must be unique to be able to successfully add the
     * property.
     *
    ...
    /** \brief Retrieves the type and shape information of the parameter associated with the given
     * parameter name.
     *
     * This function retrieves the type and shape of the parameter associated with the given
     * parameter name. The parameter must exist in the checkpoint state to be able to retrieve its
     * type and shape information successfully.
     *
    ...
    ORT_API2_STATUS(GetParameterTypeAndShape, _In_ const OrtCheckpointState* checkpoint_state,
                    _In_ const char* parameter_name,
                    _Outptr_ OrtTensorTypeAndShapeInfo** parameter_type_and_shape);

    /** \brief Updates the data associated with the model parameter in the checkpoint state for the
     * given parameter name.
     *
     * This function updates a model parameter in the checkpoint state with the given parameter
     * data. The training session must be already created with the checkpoint state that contains
     * the parameter being updated. The given parameter is copied over to the registered device for
     * the training session. The parameter must exist in the checkpoint state to be able to update
     * it successfully.
     *
     * \param[in] checkpoint_state The checkpoint state.
    ...
    /** \brief Gets the data associated with the model parameter from the checkpoint state for the
     * given parameter name.
     *
     * This function retrieves the model parameter data from the checkpoint state for the given
     * parameter name. The parameter is copied over and returned as an OrtValue. The training
     * session must be already created with the checkpoint state that contains the parameter being
     * retrieved. The parameter must exist in the checkpoint state to be able to retrieve it
     * successfully.
     *
     * \param[in] checkpoint_state The checkpoint state.
     * \param[in] parameter_name Name of the parameter being retrieved.
