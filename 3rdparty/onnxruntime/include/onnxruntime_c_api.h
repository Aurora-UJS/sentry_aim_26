 ...
 *
 * ONNX Runtime is a high-performance inference and training graph execution engine for deep
 * learning models.
 *
...
typedef enum ONNXTensorElementDataType {
    ...
    ONNX_TENSOR_ELEMENT_DATA_TYPE_FLOAT8E4M3FN,  // Non-IEEE floating-point format based on IEEE754
                                                 // single-precision
    ONNX_TENSOR_ELEMENT_DATA_TYPE_FLOAT8E4M3FNUZ,  // Non-IEEE floating-point format based on
                                                   // IEEE754 single-precision
    ONNX_TENSOR_ELEMENT_DATA_TYPE_FLOAT8E5M2,     // Non-IEEE floating-point format based on IEEE754
                                                  // single-precision
    ONNX_TENSOR_ELEMENT_DATA_TYPE_FLOAT8E5M2FNUZ  // Non-IEEE floating-point format based on IEEE754
                                                  // single-precision
...
typedef struct OrtAllocator {
    ...
    /**
     * @brief Optional allocation function to use for memory allocations made during session
     * initialization. Use this function if you want to separate allocations made by ORT during
     * Run() calls from those made during session initialization. This allows for separate memory
     * management strategies for these allocations.
     */
...
typedef struct OrtMIGraphXProviderOptions {
    ...
    int migraphx_use_native_calibration_table;  // MIGraphx INT8 cal table. Default 0 = false,
                                                // noznero = true
...
struct OrtApiBase {
    ...
    /** \brief Returns a null terminated string of the version of the Onnxruntime library (eg:
     * "1.8.1")
     *
...
typedef OrtStatus*(ORT_API_CALL* RegisterCustomOpsFn)(OrtSessionOptions* options,
                                                      const OrtApiBase* api);

/** \brief Callback function for RunAsync
 *
 * \param[in] user_data User specific data that passed back to the callback
 * \param[out] outputs On succeed, outputs host inference results, on error, the value will be
 * nullptr
 ...
 */
typedef void (*RunAsyncCallbackFn)(void* user_data, OrtValue** outputs, size_t num_outputs,
                                   OrtStatusPtr status);
...
struct OrtApi {
     ...
     *
     * \note Invoking this function will return the same instance of the environment as that
     * returned by a previous call to another env creation function; all arguments to this function
     * will be ignored.
     ...
     * \note Invoking this function will return the same instance of the environment as that
     * returned by a previous call to another env creation function; all arguments to this function
     * will be ignored. If you want to provide your own logging function, consider setting it using
     * the SetUserLoggingFunction API instead.
     ...
     * `param` parameter to `logging_function`. This parameter is optional.
     * \param[in] log_severity_level The log severity level.
     * \param[in] logid The log identifier.
     * \param[out] out Returned newly created OrtEnv. Must be freed with OrtApi::ReleaseEnv
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     */
    ORT_API2_STATUS(CreateEnvWithCustomLogger, _In_ OrtLoggingFunction logging_function,
                    _In_opt_ void* logger_param, _In_ OrtLoggingLevel log_severity_level,
     ...
     * \note Memory pattern optimization is only available when Sequential Execution mode is enabled
     ...
     *
     * Please see
     * https://onnxruntime.ai/docs/performance/model-optimizations/graph-optimizations.html for an
     ...
     * dimensions stored in the ::OrtTensorTypeAndShapeInfo
     ...
     * to null terminated strings of the dimension names
     ...
     * set to nullptr if the custom metadata map is empty.
     ...
     * lengths.
     * \param[out] buffer Returns an array of non-null terminated UTF-8 strings. The number of
     * strings stored is returned in the count parameter. This buffer is allocated using `allocator`
     * and must be freed using it.
     ...
     * \param[out] count Number of strings returned. If `binding_ptr` has no bound outputs, zero is
     * returned, no memory allocation is performed and buffer and lengths are set to nullptr.
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     */
     ...
     * location relative to its shape
     * \param[in] location_values_count Number of elements in location_values. Must match the number
     * of elements in the tensor's shape.
     ...
     * \param[in] logging_function A pointer to a logging function.
     * \param[in] logger_param A pointer to arbitrary data passed as the ::OrtLoggingFunction
     * `param` parameter to `logging_function`.
     * \param[in] log_severity_level The log severity level.
     * \param[in] logid The log identifier.
     * \param[in] tp_options
     ...
     * \note This option is not needed if the models used have no denormals. Having no denormals is
     * recommended as this option may hurt model accuracy.
     *
     * \param[in] tp_options
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     ...
     *
     * Supported keys are (See https://onnxruntime.ai/docs/get-started/with-c.html for details on
     ...
     * default. "max_power_of_two_extend_bytes": The maximum enxtend size if arena strategy is
     * `kNextPowerOfTwo`. It is not an allocation limit, it is only a limit for extention when
     * requested byte is less than the limit. When requested bytes is more than the limit, allocator
     * will still return as requested. Use -1 to allow ORT to choose the default 1GB for
     * max_power_of_two_extend_bytes. Ultimately, the allocation size is determined by the
     * allocation memory request. Further allocation sizes are governed by the arena extend
     * strategy.
     *
     ...
     *
     * Please refer to
     * https://onnxruntime.ai/docs/execution-providers/TensorRT-ExecutionProvider.html#cc to know
     * the available keys and values. Key should be in null terminated string format of the member
     * of ::OrtTensorRTProviderOptionsV2 and value should be its related range. Recreates the
     * options and only sets the supplied values.
     *
     ...
     * keys
     * \param[in] provider_options_values Array of UTF-8 null-terminated string for provider options
     * values
     ...
     * \param ptr - is a UTF-8 null terminated string allocated using 'allocator'. The caller is
     * responsible for using the same allocator to free it.
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     */
     ...
     * \param[in,out] allocator allocator to use when performing an allocation. Allocation will be
     * performed by FillSparseTensor<Format>() APIs. The lifespan of the allocator instance must
     * eclipse the lifespan this sparse tensor instance as the same allocator will be used to free
     * memory.
     ...
     * CPU and will only be copied into a CPU allocated buffer.
     ...
     * \param[in] indices_data pointer to a location of indices data. Shape will determine the
     * length of the indices data.
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     */
     ...
     * tensor with no non-zero values, pass nullptr
     * \param[in] dense_shape shape of the original dense tensor
     * \param[in] dense_shape_len number of shape dimensions being passed
     * \param[in] values_shape shape of the values data. To create a fully sparse tensor with no
     * non-zero values, pass {0} shape.
     ...
     * \param[in,out] indices_data pointer to a user pre-allocated buffer or nullptr for fully
     * sparse tensors.
     * \param[in] indices_num  number of COO indices. Should either be 0 for fully sparse tensors,
     * be equal to the number of nnz values specified to
     * OrtApi::CreateSparseTensorWithValuesAsOrtValue for 1-D {nnz} indices or be twice as number of
     * nnz values for a  2-D indices {nnz, 2}
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     */
     ...
     * \param[in,out] ort_value ::OrtValue instance constructed with
     * OrtApi::CreateSparseTensorWithValuesAsOrtValue
     * \param[in,out] inner_data pointer to a user pre-allocated buffer or nullptr for fully sparse
     * tensors.
     * \param[in] inner_num  number of inner CSR indices. Should either be 0 for fully sparse
     * tensors or be equal to the number of nnz values specified to
     * OrtApi::CreateSparseTensorWithValuesAsOrtValue.
     * \param[in,out] outer_data pointer to user pre-allocated buffer or nullptr for fully sparse
     * tensors.
     ...
     * \param[in,out] indices_data pointer to user pre-allocated buffer or nullptr for fully sparse
     * tensors.
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     */
     ...
     * the sparse tensor does not contain.
     ...
     * \param[out] indices Returned pointer to the indices data. Do not free the returned pointer as
     * it refers to internal data owned by the ::OrtValue
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     */
     ...
     *   \see ::OrtCustomOp
     * \param[in]  context OrtKernelContext instance
     * \param[out] out Returns pointer to a GPU compute stream that can be used to launch the custom
     * GPU kernel. If retrieving the GPU compute stream is not relevant (GPU not enabled in the
     * build, kernel partitioned to some other EP), then a nullptr is returned as the output param.
     *             Do not free or mutate the returned pointer as it refers to internal data owned by
     * the underlying session. Only use it for custom kernel launching.
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     */
     ...
     * following values are supported: "DML".
     * \param[in] version Must be ::ORT_API_VERSION.
     ...
     * member of ::OrtCUDAProviderOptionsV2 and value should be its related range. Recreates the
     * options and only sets the supplied values.
     *
    ...
    ORT_API2_STATUS(AddExternalInitializers, _In_ OrtSessionOptions* options,
                    ...
                    _In_reads_(num_initializers) const OrtValue* const* initializers,
     ...
     * \since Version 1.12.
     */
    ORT_API2_STATUS(CreateOp, _In_ const OrtKernelInfo* info, _In_z_ const char* op_name,
                    _In_z_ const char* domain, int version,
                    _In_reads_(type_constraint_count) const char** type_constraint_names,
                    _In_reads_(type_constraint_count)
                        const ONNXTensorElementDataType* type_constraint_values,
                    int type_constraint_count,
                    _In_reads_(attr_count) const OrtOpAttr* const* attr_values, int attr_count,
                    int input_count, int output_count, _Outptr_ OrtOp** ort_op);
     ...
     * Note: If an execution provider has a dedicated
     SessionOptionsAppendExecutionProvider_<provider name> function
     *       that should be used to add it.
     *
     * QNN supported keys:
     *   "backend_path": file path to QNN backend library.
     *   "profiling_level": QNN profiling level, options: "off", "basic", "detailed". Default to
     off.
     *   "profiling_file_path": QNN profiling file path if ETW not enabled.
     *   "rpc_control_latency": QNN RPC control latency.
     *   "vtcm_mb": QNN VTCM size in MB. default to 0(not set).
     *   "htp_performance_mode": QNN performance mode, options: "burst", "balanced", "default",
     "high_performance",
     *   "high_power_saver", "low_balanced", "extreme_power_saver", "low_power_saver",
     "power_saver", "sustained_high_performance". Default to "default".
     *   "qnn_saver_path": File path to the QNN Saver backend library. If specified, QNN Saver will
     be enabled and will
     *   dump QNN API calls to disk for replay/debugging. QNN Saver produces incorrect model
     inference results and
     *   may alter model/EP partitioning. Use only for debugging.
     *   "qnn_context_priority": QNN context priority, options: "low", "normal", "normal_high",
     "high". Default to "normal".
     *   "htp_graph_finalization_optimization_mode": Set the optimization mode for graph
     finalization on the HTP backend. Available options:
     *     - "0": Default.
     ...
     *   "htp_arch": The minimum HTP architecture the driver will use to select compatible QNN
     operators. Available options:
     *     - "0": Default (none).
     *     - "68"
     *     - "69"
     *     - "73"
     *     - "75"
     *   "device_id": The ID of the device to use when setting 'htp_arch'. Defaults to "0" (for
     single device). "enable_htp_fp16_precision": Only used for float32 model. Enable the float32
     model to be inferenced with fp16 precision. Otherwise, it will be fp32 precision.
           - "0": Default. With fp32 precision.
           - "1": With fp16 precision.
     *
     * SNPE supported keys:
     *   "runtime": SNPE runtime engine, options: "CPU", "CPU_FLOAT32", "GPU",
     "GPU_FLOAT32_16_HYBRID", "GPU_FLOAT16",
     ...
     *   "buffer_type": ITensor or user buffers, options: "ITENSOR", user buffer with different
     types - "TF8", "TF16", "UINT8", "FLOAT".
     *   "ITENSOR" -- default, ITensor which is float only.
     *   "TF8" -- quantized model required, "FLOAT" -- for both quantized or non-quantized model
     *   "enable_init_cache": enable SNPE init caching feature, set to 1 to enabled it. Disabled by
     default.
     ...
     *
     * Call this function to access the ::OrtTrainingApi structure that holds pointers to functions
     * that enable training with onnxruntime.
     * \note A NULL pointer will be returned and no error message will be printed if the training
     * api is not supported with this build. A NULL pointer will be returned and an error message
     * will be printed if the provided version is unsupported, for example when using a runtime
     ...
     *
     * If CANN is not available (due to a non CANN enabled build, or if CANN is not installed on the
     * system), this function will return failure.
     *
     * \param[in] options
     ...
     *                      or GetAllocatorWithDefaultOptions(), the specified allocator will be
     * used to allocate continuous buffers for output strings and lengths.
     ...
     * Semicolon isolates configurations among threads, while comma split processors where ith
     * thread expected to attach to. e.g. 1,2,3;4,5 specifies affinities for two threads, with the
     * 1st thread attach to the 1st, 2nd, and 3rd processor, and 2nd thread to the 4th and 5th. To
     * ease the configuration, an "interval" is also allowed: e.g. 1-8;8-16;17-24 orders that the
     * 1st thread runs on first eight processors, 2nd thread runs on next eight processors, and so
     * forth. Note:
     * 1. Once set, the number of thread affinities must equal to intra_op_num_threads - 1,
     *    ort does not set affinity on the main thread which is started and managed by the calling
     * app;
     * 2. For windows, ort will infer the group id from a logical processor id, for example,
     * assuming there are two groups with each has 64 logical processors, an id of 64 will be
     * inferred as the last processor of the 1st group, while 65 will be interpreted as the 1st
     * processor of the second group. Hence 64-65 is an invalid configuration, because a windows
     * thread cannot be attached to processors across group boundary.
     *
     *  \since Version 1.14
     */
     ...
     *
     * Loads a shared library (.dll on windows, .so on linux, etc) named 'library_name' and looks
     *for this entry point: OrtStatus* RegisterCustomOps(OrtSessionOptions * options, const
     *OrtApiBase* api); It then passes in the provided session options to this function along with
     *the api base.
     *
     * The handle to the loaded library is automatically released by ORT when the last OrtSession
     *that references the library handle is released. If no OrtSession is created, then the library
     *handle is released when the provided OrtSessionOptions is released.
     *
     * \param[in] options The session options.
     * \param[in] library_name The name of the shared library to load and register. Refer to
     *OS-specific dynamic library loading utilities (e.g., LoadLibraryEx on Windows or dlopen on
     *Linux/MacOS) for information on the format of library names and search paths.
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     * \since Version 1.14
     */
     ...
     *
     * The library containing the function must either be linked against or previously loaded by the
     * executable.
     *
     * If you want ONNX Runtime to load the library and manage its lifetime, use
     * RegisterCustomOpsLibrary_V2.
     *
     * RegisterCustomOpsUsingFunction can be used in scenarios where it may not be possible for ONNX
     * Runtime to load the library from a path. e.g. mobile platforms where the library must be
     * linked into the app.
     *
     * The registration function must have the signature of RegisterCustomOpsFn:
     *    OrtStatus* (*fn)(OrtSessionOptions* options, const OrtApiBase* api);
     *
     * See https://onnxruntime.ai/docs/reference/operators/add-custom-op.html for details on how the
     * registration function should be implemented.
     *
     * \param[in] options OrtSessionOptions that is passed through as the first argument in the call
     ...
     * \param[in] index The index of the input name to get. Returns a failure status if
     * out-of-bounds.
     * \param[out] out Memory location into which to write the UTF-8 null-terminated string
     * representing the input's name.
     ...
     *
     * \param[in] info An instance of ::OrtKernelInfo.
     * \param[in] index The index of the output name to get. Returns a failure status if
     * out-of-bounds.
     * \param[out] out Memory location into which to write the UTF-8 null-terminated string
     * representing the output's name.
     * \param[in,out] size Pointer to the size of the `out` buffer. See above comments for details.
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     * \since Version 1.14
     ...
     * \param[out] type_info Pointer set to the resulting ::OrtTypeInfo. Must be freed with
     ...
     * \param[out] out Returns newly created ::OrtValue. Must be freed with OrtApi::ReleaseValue,
     *                 which will also free internal tensor state allocated with the provided
     * allocator.
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     */
     ...
     * \param[in] config_value Pointer to memory where the null-terminated UTF-8 string value will
     * be stored.
     * \param[in,out] size Pointer to the size of the `config_value` buffer. See above comments for
     * details.
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     * \since Version 1.14
     ...
     *
     * \param[out] out Newly created ::OrtDnnlProviderOptions. Must be released with
     * OrtApi::ReleaseDnnlProviderOptions
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     *
     * \since Version 1.15.
     ...
     * and a failure status is returned.
     *
     * Can be used in a custom operator's CreateKernel callback to get the name of the operator's
     ...
     * \since Version 1.15
     */
    ORT_API2_STATUS(KernelInfo_GetNodeName, _In_ const OrtKernelInfo* info, _Out_ char* out,
                    _Inout_ size_t* size);
     ...
     *
     * \param[in] info An instance of ::OrtKernelInfo.
     * \param[out] logger Pointer set to the session's ::OrtLogger. Owned by ONNX Runtime, so do not
     * free.
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     * \since Version 1.15
     ...
     * \param[out] logger Pointer set to the kernel context's ::OrtLogger. Owned by ONNX Runtime, so
     * do not free.
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     * \since Version 1.15
     ...
     *
     * Only messages with a severity level equal or greater than the ::OrtLogger's logging severity
     * level are logged. Use OrtApi::Logger_GetLoggingSeverityLevel to get the ::OrtLogger's logging
     * severity level.
     *
     * Can be used in custom operators to log messages with the logger retrieved via
     * OrtApi::KernelInfo_GetLogger.
     *
     * \param[in] logger The ::OrtLogger instance.
     * \param[in] log_severity_level The message's severity level.
     * \param[in] message The message to log.
     * \param[in] file_path The filepath of the file in which the message is logged. Usually the
     * value of ORT_FILE.
     * \param[in] line_number The file line number in which the message is logged. Usually the value
     * of __LINE__.
     * \param[in] func_name The name of the function in which the message is logged. Usually the
     * value of __FUNCTION__.
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     * \since Version 1.15
     */
    ORT_API2_STATUS(Logger_LogMessage, _In_ const OrtLogger* logger,
                    OrtLoggingLevel log_severity_level, _In_z_ const char* message,
                    _In_z_ const ORTCHAR_T* file_path, int line_number,
                    _In_z_ const char* func_name);

    /** \brief Get the logging severity level of the ::OrtLogger.
     *
     * Can be used in a custom operator to get the logging serverity level of the ::OrtLogger
     ...
     * The OrtOptionalTypeInfo also has a nested ::OrtTypeInfo that describes the type of the
     * optional value.
     ...
     *
     * So the picture: ::OrtTypeInfo -> ::OrtOptionalTypeInfo -> ::OrtTypeInfo (describes the type
     * that can be supplied in place of the optional type when creating the actual ::OrtValue).
     *
     * \param[in] type_info
     ...
     * The OrtOptionalTypeInfo has a nested ::OrtTypeInfo that describes the type of the optional
     * value.
     * ::OrtOptionalTypeInfo type can only appear within model metadata to describe inputs/outputs.
     * The actual OrtValues that are supplied in place of optional type inputs should contain
    ...
    ORT_API2_STATUS(GetOptionalContainedTypeInfo,
                    _In_ const OrtOptionalTypeInfo* optional_type_info, _Outptr_ OrtTypeInfo** out);
    ...
    /** \brief Get Allocator from KernelContext for a specific memoryInfo. Please use C API
     ...
     *
     * \param[out] out Newly created ::OrtROCMProviderOptions. Must be released with
     * OrtApi::ReleaseROCMProviderOptions
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     *
     * \since Version 1.16.
     ...
     *  This API enhance CreateAndRegisterAllocator that it can create an allocator with specific
     * type, not just CPU allocator Enables sharing the allocator between multiple sessions that use
     * the same env instance. Lifetime of the created allocator will be valid for the duration of
     * the environment. Returns an error if an allocator with the same ::OrtMemoryInfo is already
     * registered.
     *  \param[in] env OrtEnv instance
    ...
    ORT_API2_STATUS(CreateAndRegisterAllocatorV2, _Inout_ OrtEnv* env,
                    _In_ const char* provider_type, _In_ const OrtMemoryInfo* mem_info,
     ...
     *             On calling RunAsync, output[i] could either be a null or a pointer to a
     * preallocated OrtValue. Later, the output array will be passed to run_async_callback with all
     * null(s) filled with valid OrtValue pointer(s) allocated by onnxruntime. NOTE: it is
     * customer's duty to finally release the output array and each of its member, regardless of
     * whether the member (OrtValue*) is allocated by onnxruntime or preallocated by the customer.
    ...
    /**
     * Update TensorRT EP provider option where its data type is pointer, for example
     * 'user_compute_stream'. If the data type of the provider option can be represented by string
     * please use UpdateTensorRTProviderOptions.
     *
     * Note: It's caller's responsibility to properly manage the lifetime of the instance pointed by
     * this pointer.
     *
     * \param tensorrt_options - OrtTensorRTProviderOptionsV2 instance
    ...
    ORT_API2_STATUS(UpdateTensorRTProviderOptionsWithValue,
                    _Inout_ OrtTensorRTProviderOptionsV2* tensorrt_options, _In_ const char* key,
                    _In_ void* value);
    ...
    ORT_API2_STATUS(GetTensorRTProviderOptionsByName,
                    _In_ const OrtTensorRTProviderOptionsV2* tensorrt_options, _In_ const char* key,
                    _Outptr_ void** ptr);

    /**
     * Update CUDA EP provider option where its data type is pointer, for example
     * 'user_compute_stream'. If the data type of the provider option can be represented by string
     * please use UpdateCUDAProviderOptions.
     *
     * Note: It's caller's responsibility to properly manage the lifetime of the instance pointed by
     * this pointer.
     *
    ...
    ORT_API2_STATUS(UpdateCUDAProviderOptionsWithValue,
                    _Inout_ OrtCUDAProviderOptionsV2* cuda_options, _In_ const char* key,
                    _In_ void* value);
     ...
     * \since Version 1.16.
     */
    ORT_API2_STATUS(KernelContext_GetResource, _In_ const OrtKernelContext* context,
                    _In_ int resouce_version, _In_ int resource_id, _Outptr_ void** resource);

    /** \brief Set user logging function
     *
     *  By default the logger created by the CreateEnv* functions is used to create the session
     * logger as well. This function allows a user to override this default session logger with a
     * logger of their own choosing. This way the user doesn't have to create a separate environment
     * with a custom logger. This addresses the problem when the user already created an env but now
     * wants to use a different logger for a specific session (for debugging or other reasons).
     *
     * \param[in] options
     * \param[in] user_logging_function A pointer to a logging function.
     * \param[in] user_logging_param A pointer to arbitrary data passed as the ::OrtLoggingFunction
     * `param` parameter to `user_logging_function`. This parameter is optional.
     *
     * \snippet{doc} snippets.dox OrtStatus Return Value
     *
     * \since Version 1.17.
     ...
     * \since Version 1.17.
     */
    ORT_API2_STATUS(ShapeInferContext_GetInputCount, _In_ const OrtShapeInferContext* context,
                    _Out_ size_t* out);
     ...
     * \since Version 1.17.
     */
    ORT_API2_STATUS(ShapeInferContext_GetInputTypeShape, _In_ const OrtShapeInferContext* context,
                    _In_ size_t index, _Outptr_ OrtTensorTypeAndShapeInfo** info);

    /**
     * Get attribute from OrtShapeInferContext. Note that OrtShapeInferContext is a per-node
     * context, one could only read attribute from current node.
     *
     * \param[in] context
     ...
     * \since Version 1.17.
     */
    ORT_API2_STATUS(ShapeInferContext_SetOutputTypeShape, _In_ const OrtShapeInferContext* context,
     ...
     * \param[out] out Number of bytes required to save the data when the call failed, or the real
     * number of bytes saved to data on success
     *
     * \since Version 1.17.
     */
    ORT_API2_STATUS(ReadOpAttr, _In_ const OrtOpAttr* op_attr, _In_ OrtOpAttrType type,
                    _Inout_ void* data, _In_ size_t len, _Out_ size_t* out);

    /** \brief Set whether to use deterministic compute.
     *
     * Default is false. If set to true, this will enable deterministic compute for GPU kernels
     ...
     * \param[in] num_batch Number of batches by which the "total" is to be divided in maximum. When
     * zero, there is no limit
     * \param[in] usr_data User data to be passed back to fn
     *
     * \since Version 1.17.
     */
    ORT_API2_STATUS(KernelContext_ParallelFor, _In_ const OrtKernelContext* context,
     ...
     *
     * If VitisAI is not available (due to a non VitisAI enabled build, or if VitisAI is not
     * installed on the system), this function will return failure.
     *
     * \param[in] options
     * \param[in] provider_options_keys
    ...
    /** \brief Get scratch buffer from the corresponding allocator under the sepcific OrtMemoryInfo
     * object. NOTE: callers are responsible to release this scratch buffer from the corresponding
     * allocator
     *  \param[in] context OrtKernelContext instance
     *  \param[in] mem_info OrtMemoryInfo instance
    ...
    ORT_API2_STATUS(KernelContext_GetScratchBuffer, _In_ const OrtKernelContext* context,
                    _In_ const OrtMemoryInfo* mem_info, _In_ size_t count_or_bytes,
                    _Outptr_ void** out);

    /** \brief Get allocator from KernelInfo for a specific memory type. Please use C API
     * ReleaseAllocator to release out object
     *
     * \param[in] info OrtKernelInfo instance
     ...
     *
     * The function will find the initialized TensorProtos with external data in the graph with the
     * provided external file names and the file content in memory. The API gets the external file
     * name, offset, data length from TensorProto, and locate the tensor data from the file in
     * memory buffer. It creates a Tensor to replace the existing Tensor in graph. The replacement
     ...
     * \param[in] external_initializer_file_names Array of null terminated UTF-8 encoded strings of
     * the file names which holds the external initializers.
     * \param[in] external_initializer_file_buffer_array Array of pointers to the buffer of the file
     * content. The buffer can be freed after session creation.
     * \param[in] external_initializer_file_lengths Array of size_t to indicate the length of file
     * content
...
//              The homogeneity of the variadic input or output determines whether all operands must
...
struct OrtCustomOp {
    ...
    // Returns true (non-zero) if all arguments of a variadic input have to be of the same type
    // (homogeneous), and false (zero) otherwise. Applicable only for custom ops that have a
    // variadic input.
    ...
    // Returns true (non-zero) if all outputs values of a variadic output have to be of the same
    // type (homogeneous), and false (zero) otherwise. Applicable only for custom ops that have a
    // variadic output.
    ...
    OrtStatusPtr(ORT_API_CALL* InferOutputShapeFn)(_In_ const struct OrtCustomOp* op,
    ...
    // Callers will provide 2 raw int* and pass in their address, this function will fill these 2
    // arrays when return, output (*output_index)[i] may reuse the input (*input_index[i]). The
    // return value is the size of these 2 arrays. Callers are responsible to delete these 2 arrays
 ...
 */
ORT_API_STATUS(OrtSessionOptionsAppendExecutionProvider_ROCM, _In_ OrtSessionOptions* options,
               int device_id);
 ...
 */
ORT_API_STATUS(OrtSessionOptionsAppendExecutionProvider_Dnnl, _In_ OrtSessionOptions* options,
               int use_arena);

/*
 * This is the old way to add the TensorRT provider to the session, please use
 * SessionOptionsAppendExecutionProvider_TensorRT_V2 above to access the latest functionality This
 * function always exists, but will only succeed if Onnxruntime was built with TensorRT support and
 * the TensorRT provider shared library exists
 *
 * \param device_id CUDA device id, starts from zero.
 */
