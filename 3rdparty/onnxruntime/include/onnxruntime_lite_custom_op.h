...
// which will be inferred by functions' signature, as long as their argument list has types
// supported here. Input could be:
...
// Further, the header also has utility for a simple custom struct, where resources could be kept,
// to be registered as a custom op. For concrete examples, please search keyword "LiteCustomOpTest"
// under "<cloned_src_dir>/onnxruntime/test/". Note - all APIs in this header are ABI.
...
public:
    ArgBase(OrtKernelContext* ctx, size_t indice, bool is_input)
        : ctx_(ctx), indice_(indice), is_input_(is_input) {}
    virtual ~ArgBase() {};
...
public:
    ...
    operator bool() const { return shape_.has_value(); }

    const std::vector<int64_t>& Shape() const {
        if (!shape_.has_value()) {
            ORT_CXX_API_THROW("tensor shape is not yet initialized",
                              OrtErrorCode::ORT_RUNTIME_EXCEPTION);
        }
        return shape_.value();
    }

    ONNXTensorElementDataType Type() const { return type_; }
    ...
    bool IsCpuTensor() const { return strcmp("Cpu", mem_type_) == 0; }
...
struct Span {
    ...
    T operator[](size_t indice) const { return data_[indice]; }
...
public:
    ...
    Tensor(OrtKernelContext* ctx, size_t indice, bool is_input)
        ...
        if (is_input_) {
            if (indice >= ctx_.GetInputCount()) {
                ORT_CXX_API_THROW("invalid indice for Ort::Custom::Tensor",
    ...
    const TT* Data() const { return reinterpret_cast<const TT*>(const_value_.GetTensorRawData()); }
    ...
    const void* DataRaw() const override { return reinterpret_cast<const void*>(Data()); }
...
public:
    ...
    Tensor(OrtKernelContext* ctx, size_t indice, bool is_input)
        ...
        if (is_input_) {
            ...
            if (num_strings) {
                ...
                const_value.GetStringTensorContent(static_cast<void*>(chars.data()), num_chars,
    ...
    const strings& Data() const { return input_strings_; }
    ...
    const Span<std::string>& AsSpan() {
        ORT_CXX_API_THROW("span for TensorT of string not implemented",
                          OrtErrorCode::ORT_RUNTIME_EXCEPTION);
    }
...
public:
    ...
    Tensor(OrtKernelContext* ctx, size_t indice, bool is_input)
        ...
        if (is_input_) {
            ...
            if (num_strings) {
                std::vector<size_t> offsets(num_strings);
                const_value.GetStringTensorContent(static_cast<void*>(chars_.data()), num_chars,
                                                   offsets.data(), offsets.size());
                offsets.push_back(num_chars);
                for (size_t i = 0; i < num_strings; ++i) {
                    input_string_views_.emplace_back(chars_.data() + offsets[i],
    ...
    const Span<std::string_view>& AsSpan() {
        ORT_CXX_API_THROW("span for TensorT of string view not implemented",
                          OrtErrorCode::ORT_RUNTIME_EXCEPTION);
    }
...
struct TensorArray : public ArgBase {
    TensorArray(OrtKernelContext* ctx, size_t start_indice, bool is_input)
        : ArgBase(ctx, start_indice, is_input) {
        if (is_input) {
            ...
            for (size_t ith_input = start_indice; ith_input < input_count; ++ith_input) {
                ...
                switch (type) {
                    ...
                    case ONNX_TENSOR_ELEMENT_DATA_TYPE_STRING:
                        tensor =
                            std::make_unique<Custom::Tensor<std::string>>(ctx, ith_input, true);
                        break;
                    default:
    ...
    T* AllocateOutput(size_t ith_output, const std::vector<int64_t>& shape) {
        ...
        auto tensor =
            std::make_unique<Tensor<T>>(ctx_.GetOrtKernelContext(), indice_ + ith_output, false);
    ...
    Tensor<std::string>& AllocateStringTensor(size_t ith_output) {
        // ith_output is the indice of output relative to the tensor array
        // indice_ + ith_output is the indice relative to context
        auto tensor = std::make_unique<Tensor<std::string>>(ctx_.GetOrtKernelContext(),
                                                            indice_ + ith_output, false);
...
1. DO NOT cast OrtLiteCustomOp to OrtCustomOp and release since there is no virtual destructor in
the hierachy.
2. OrtLiteCustomFunc and OrtLiteCustomStruct, as two sub-structs, can be released in form of
OrtLiteCustomOp since all members are kept in the OrtLiteCustomOp, hence memory could still be
recycled properly. Further, OrtCustomOp is a c struct bearing no v-table, so offspring structs are
by design to be of zero virtual functions to maintain cast safety.
*/
struct OrtLiteCustomOp : public OrtCustomOp {
    ...
    static typename std::enable_if<sizeof...(Ts) == 0, std::tuple<>>::type CreateTuple(
        OrtKernelContext*, ArgPtrs&, size_t, size_t, const std::string&) {
        return std::make_tuple();
    }

    template <size_t ith_input, size_t ith_output, typename T, typename... Ts>
    static typename std::enable_if<std::is_same<T, OrtKernelContext*>::value,
                                   std::tuple<T, Ts...>>::type
    CreateTuple(OrtKernelContext* context, ArgPtrs& args, size_t num_input, size_t num_output,
                const std::string& ep) {
        std::tuple<T> current = std::tuple<OrtKernelContext*>{context};
        auto next =
            CreateTuple<ith_input, ith_output, Ts...>(context, args, num_input, num_output, ep);
...
#ifdef ORT_CUDA_CTX
    template <size_t ith_input, size_t ith_output, typename T, typename... Ts>
    static typename std::enable_if<std::is_same<T, const CudaContext&>::value,
                                   std::tuple<T, Ts...>>::type
    CreateTuple(OrtKernelContext* context, ArgPtrs& args, size_t num_input, size_t num_output,
                const std::string& ep) {
...
#ifdef ORT_ROCM_CTX
    template <size_t ith_input, size_t ith_output, typename T, typename... Ts>
    static typename std::enable_if<std::is_same<T, const RocmContext&>::value,
                                   std::tuple<T, Ts...>>::type
    CreateTuple(OrtKernelContext* context, ArgPtrs& args, size_t num_input, size_t num_output,
                const std::string& ep) {
    ...
    template <size_t ith_input, size_t ith_output, typename T, typename... Ts>
    static typename std::enable_if<std::is_same<T, const TensorArray*>::value,
                                   std::tuple<T, Ts...>>::type
    CreateTuple(OrtKernelContext* context, ArgPtrs& args, size_t num_input, size_t num_output,
                const std::string& ep) {
        ...
        auto next =
            CreateTuple<ith_input + 1, ith_output, Ts...>(context, args, num_input, num_output, ep);
        return std::tuple_cat(current, next);
    }
    ...
    CreateTuple(OrtKernelContext* context, ArgPtrs& args, size_t num_input, size_t num_output,
        ...
        std::tuple<T> current = std::tuple<T>{reinterpret_cast<T>(args.back().get())};
        auto next =
            CreateTuple<ith_input, ith_output + 1, Ts...>(context, args, num_input, num_output, ep);
        return std::tuple_cat(current, next);
    }
...
#define CREATE_TUPLE_INPUT(data_type)                                                              \
    template <size_t ith_input, size_t ith_output, typename T, typename... Ts>                     \
    static typename std::enable_if<std::is_same<T, const Custom::Tensor<data_type>*>::value,       \
                                   std::tuple<T, Ts...>>::type                                     \
    CreateTuple(OrtKernelContext* context, ArgPtrs& args, size_t num_input, size_t num_output,     \
                const std::string& ep) {                                                           \
        args.push_back(std::make_unique<Custom::Tensor<data_type>>(context, ith_input, true));     \
        std::tuple<T> current = std::tuple<T>{reinterpret_cast<T>(args.back().get())};             \
        auto next = CreateTuple<ith_input + 1, ith_output, Ts...>(context, args, num_input,        \
                                                                  num_output, ep);                 \
        return std::tuple_cat(current, next);                                                      \
    }                                                                                              \
    template <size_t ith_input, size_t ith_output, typename T, typename... Ts>                     \
    ...
    static typename std::enable_if<                                                                \
        std::is_same<T, std::optional<const Custom::Tensor<data_type>*>>::value,                   \
        std::tuple<T, Ts...>>::type                                                                \
    CreateTuple(OrtKernelContext* context, ArgPtrs& args, size_t num_input, size_t num_output,     \
                const std::string& ep) {                                                           \
        if (ith_input < num_input) {                                                               \
            args.push_back(std::make_unique<Custom::Tensor<data_type>>(context, ith_input, true)); \
            std::tuple<T> current =                                                                \
                std::tuple<T>{reinterpret_cast<Custom::Tensor<data_type>*>(args.back().get())};    \
            auto next = CreateTuple<ith_input + 1, ith_output, Ts...>(context, args, num_input,    \
                                                                      num_output, ep);             \
            return std::tuple_cat(current, next);                                                  \
        } else {                                                                                   \
    ...
    template <size_t ith_input, size_t ith_output, typename T, typename... Ts>                     \
    static typename std::enable_if<std::is_same<T, const Custom::Span<data_type>*>::value,         \
                                   std::tuple<T, Ts...>>::type                                     \
    CreateTuple(OrtKernelContext* context, ArgPtrs& args, size_t num_input, size_t num_output,     \
                const std::string& ep) {                                                           \
        if ("CPUExecutionProvider" != ep) {                                                        \
            ORT_CXX_API_THROW("span input could only be applied to CPU EP",                        \
                              OrtErrorCode::ORT_RUNTIME_EXCEPTION);                                \
        }                                                                                          \
        args.push_back(std::make_unique<Custom::Tensor<data_type>>(context, ith_input, true));     \
        std::tuple<T> current = std::tuple<T>{                                                     \
            &reinterpret_cast<Custom::Tensor<data_type>*>(args.back().get())->AsSpan()};           \
        auto next = CreateTuple<ith_input + 1, ith_output, Ts...>(context, args, num_input,        \
                                                                  num_output, ep);                 \
        return std::tuple_cat(current, next);                                                      \
    }                                                                                              \
    ...
    template <size_t ith_input, size_t ith_output, typename T, typename... Ts>                     \
    static typename std::enable_if<                                                                \
        std::is_same<T, std::optional<const Custom::Span<data_type>*>>::value,                     \
        std::tuple<T, Ts...>>::type                                                                \
    CreateTuple(OrtKernelContext* context, ArgPtrs& args, size_t num_input, size_t num_output,     \
                const std::string& ep) {                                                           \
        if (ith_input < num_input) {                                                               \
    ...
    CreateTuple(OrtKernelContext* context, ArgPtrs& args, size_t num_input, size_t num_output,     \
                const std::string& ep) {                                                           \
        if ("CPUExecutionProvider" != ep) {                                                        \
            ORT_CXX_API_THROW("scalar input could only be applied to CPU EP",                      \
                              OrtErrorCode::ORT_RUNTIME_EXCEPTION);                                \
        }                                                                                          \
        args.push_back(std::make_unique<Custom::Tensor<data_type>>(context, ith_input, true));     \
        std::tuple<T> current = std::tuple<T>{                                                     \
            reinterpret_cast<Custom::Tensor<data_type>*>(args.back().get())->AsScalar()};          \
        auto next = CreateTuple<ith_input + 1, ith_output, Ts...>(context, args, num_input,        \
                                                                  num_output, ep);                 \
        return std::tuple_cat(current, next);                                                      \
    }                                                                                              \
    template <size_t ith_input, size_t ith_output, typename T, typename... Ts>                     \
    static typename std::enable_if<std::is_same<T, std::optional<data_type>>::value,               \
                                   std::tuple<T, Ts...>>::type                                     \
    CreateTuple(OrtKernelContext* context, ArgPtrs& args, size_t num_input, size_t num_output,     \
                const std::string& ep) {                                                           \
        if (ith_input < num_input) {                                                               \
...
#define CREATE_TUPLE_OUTPUT(data_type)                                                             \
    template <size_t ith_input, size_t ith_output, typename T, typename... Ts>                     \
    static typename std::enable_if<std::is_same<T, Custom::Tensor<data_type>*>::value,             \
                                   std::tuple<T, Ts...>>::type                                     \
    CreateTuple(OrtKernelContext* context, ArgPtrs& args, size_t num_input, size_t num_output,     \
                const std::string& ep) {                                                           \
        args.push_back(std::make_unique<Custom::Tensor<data_type>>(context, ith_output, false));   \
        std::tuple<T> current = std::tuple<T>{reinterpret_cast<T>(args.back().get())};             \
        auto next = CreateTuple<ith_input, ith_output + 1, Ts...>(context, args, num_input,        \
                                                                  num_output, ep);                 \
        return std::tuple_cat(current, next);                                                      \
    }                                                                                              \
    template <size_t ith_input, size_t ith_output, typename T, typename... Ts>                     \
    ...
    template <size_t ith_input, size_t ith_output, typename T, typename... Ts>                     \
    static                                                                                         \
        typename std::enable_if<std::is_same<T, std::optional<Custom::Tensor<data_type>*>>::value, \
                                std::tuple<T, Ts...>>::type                                        \
        CreateTuple(OrtKernelContext* context, ArgPtrs& args, size_t num_input, size_t num_output, \
                    const std::string& ep) {                                                       \
        if (ith_output < num_output) {                                                             \
            args.push_back(                                                                        \
                std::make_unique<Custom::Tensor<data_type>>(context, ith_output, false));          \
            std::tuple<T> current =                                                                \
                std::tuple<T>{reinterpret_cast<Custom::Tensor<data_type>*>(args.back().get())};    \
            auto next = CreateTuple<ith_input, ith_output + 1, Ts...>(context, args, num_input,    \
                                                                      num_output, ep);             \
...
#define CREATE_TUPLE(data_type)   \
    ...
    static typename std::enable_if<0 == sizeof...(Ts)>::type ParseArgs(
        std::vector<ONNXTensorElementDataType>&, std::vector<ONNXTensorElementDataType>&) {}

    template <typename T, typename... Ts>
    static typename std::enable_if<0 <= sizeof...(Ts) &&
                                   std::is_same<T, OrtKernelContext*>::value>::type
    ParseArgs(std::vector<ONNXTensorElementDataType>& input_types,
    ...
    template <typename T, typename... Ts>
    static typename std::enable_if<0 <= sizeof...(Ts) &&
                                   std::is_same<T, const CudaContext&>::value>::type
    ParseArgs(std::vector<ONNXTensorElementDataType>& input_types,
              std::vector<ONNXTensorElementDataType>& output_types) {
        ParseArgs<Ts...>(input_types, output_types);
    }
    ...
    template <typename T, typename... Ts>
    static typename std::enable_if<0 <= sizeof...(Ts) &&
                                   std::is_same<T, const RocmContext&>::value>::type
    ParseArgs(std::vector<ONNXTensorElementDataType>& input_types,
              std::vector<ONNXTensorElementDataType>& output_types) {
        ParseArgs<Ts...>(input_types, output_types);
    }
    ...
    template <typename T, typename... Ts>
    static typename std::enable_if<0 <= sizeof...(Ts) &&
                                   std::is_same<T, const TensorArray&>::value>::type
    ParseArgs(std::vector<ONNXTensorElementDataType>& input_types,
              std::vector<ONNXTensorElementDataType>& output_types) {
...
#define PARSE_INPUT_BASE(pack_type, onnx_type)                                                    \
    ...
    template <typename T, typename... Ts>                                                         \
    static typename std::enable_if<0 <= sizeof...(Ts) &&                                          \
                                   std::is_same<T, const std::optional<pack_type>>::value>::type  \
    ParseArgs(std::vector<ONNXTensorElementDataType>& input_types,                                \
              std::vector<ONNXTensorElementDataType>& output_types) {                             \
        input_types.push_back(onnx_type);                                                         \
        ParseArgs<Ts...>(input_types, output_types);                                              \
    ...
    template <typename T, typename... Ts>                                                         \
    static typename std::enable_if<0 <= sizeof...(Ts) &&                                          \
                                   std::is_same<T, std::optional<pack_type>>::value>::type        \
    ParseArgs(std::vector<ONNXTensorElementDataType>& input_types,                                \
              std::vector<ONNXTensorElementDataType>& output_types) {                             \
        input_types.push_back(onnx_type);                                                         \
        ParseArgs<Ts...>(input_types, output_types);                                              \
...
#define PARSE_OUTPUT(data_type, onnx_type)                                                   \
    template <typename T, typename... Ts>                                                    \
    static typename std::enable_if<0 <= sizeof...(Ts) &&                                     \
                                   std::is_same<T, Custom::Tensor<data_type>*>::value>::type \
    ParseArgs(std::vector<ONNXTensorElementDataType>& input_types,                           \
              std::vector<ONNXTensorElementDataType>& output_types) {                        \
    ...
    static typename std::enable_if<                                                          \
        0 <= sizeof...(Ts) &&                                                                \
        std::is_same<T, std::optional<Custom::Tensor<data_type>*>>::value>::type             \
    ParseArgs(std::vector<ONNXTensorElementDataType>& input_types,                           \
              std::vector<ONNXTensorElementDataType>& output_types) {                        \
        output_types.push_back(onnx_type);                                                   \
        ParseArgs<Ts...>(input_types, output_types);                                         \
...
#define PARSE_ARGS(data_type, onnx_type) \
    ...
    PARSE_ARGS(std::string_view,
               ONNX_TENSOR_ELEMENT_DATA_TYPE_STRING)  // todo - remove string_view output
    ...
    OrtLiteCustomOp(const char* op_name, const char* execution_provider,
                    ShapeInferFn shape_infer_fn, int start_ver = 1,
        ...
        OrtCustomOp::GetName = [](const OrtCustomOp* op) {
            return static_cast<const OrtLiteCustomOp*>(op)->op_name_.c_str();
        };
        OrtCustomOp::GetExecutionProviderType = [](const OrtCustomOp* op) {
            return ((OrtLiteCustomOp*)op)->execution_provider_.c_str();
        };
        ...
        OrtCustomOp::GetInputCharacteristic = [](const OrtCustomOp* op, size_t indice) {
            auto self = reinterpret_cast<const OrtLiteCustomOp*>(op);
            return self->input_types_[indice] == ONNX_TENSOR_ELEMENT_DATA_TYPE_UNDEFINED
        ...
        OrtCustomOp::GetOutputCharacteristic = [](const OrtCustomOp* op, size_t indice) {
            auto self = reinterpret_cast<const OrtLiteCustomOp*>(op);
            return self->output_types_[indice] == ONNX_TENSOR_ELEMENT_DATA_TYPE_UNDEFINED
                       ? INPUT_OUTPUT_VARIADIC
                       : INPUT_OUTPUT_OPTIONAL;
        };
...
//   void Filter(const Ort::Custom::Tensor<float>& floats_in, Ort::Custom::Tensor<float>&
//   floats_out) { ... }
// It could be registered this way:
//   Ort::CustomOpDomain v2_domain{"v2"};
//   std::unique_ptr<OrtLiteCustomOp> fil_op_ptr{Ort::Custom::CreateLiteCustomOp("Filter",
//   "CPUExecutionProvider", Filter)}; v2_domain.Add(fil_op_ptr.get());
//   session_options.Add(v2_domain);
// For the complete example, please search keyword "LiteCustomOpTest" under
// "<cloned_src_dir>/onnxruntime/test/".
...
struct OrtLiteCustomFunc : public OrtLiteCustomOp {
    ...
    OrtLiteCustomFunc(const char* op_name, const char* execution_provider, ComputeFn compute_fn,
                      ShapeInferFn shape_infer_fn = {}, int start_ver = 1,
                      int end_ver = MAX_CUSTOM_OP_END_VER)
        : OrtLiteCustomOp(op_name, execution_provider, shape_infer_fn, start_ver, end_ver) {
        ...
        OrtCustomOp::KernelCompute = [](void* op_kernel, OrtKernelContext* context) {
            ...
            auto t = CreateTuple<0, 0, Args...>(context, args, kernel->num_input_,
                                                kernel->num_output_, kernel->ep_);
            std::apply([kernel](Args const&... t_args) { kernel->compute_fn_(t_args...); }, t);
        };

        OrtCustomOp::CreateKernel = [](const OrtCustomOp* this_, const OrtApi* ort_api,
        ...
        if (shape_infer_fn_) {
            OrtCustomOp::InferOutputShapeFn = [](const OrtCustomOp* op,
    ...
    OrtLiteCustomFunc(const char* op_name, const char* execution_provider,
        ...
        OrtCustomOp::CreateKernel = [](const OrtCustomOp* this_, const OrtApi* ort_api,
            ...
            auto kernel = std::make_unique<Kernel>();
            auto me = static_cast<const MyType*>(this_);
            kernel->compute_fn_return_status_ =
                reinterpret_cast<ComputeFnReturnStatus>(me->compute_fn_return_status_);
            Ort::ThrowOnError(ort_api->KernelInfo_GetInputCount(info, &kernel->num_input_));
            Ort::ThrowOnError(ort_api->KernelInfo_GetOutputCount(info, &kernel->num_output_));
            auto self = static_cast<const OrtLiteCustomFunc*>(this_);
            kernel->ep_ = self->execution_provider_;
...
// It could be registered this way:
//   Ort::CustomOpDomain v2_domain{"v2"};
//   std::unique_ptr<OrtLiteCustomOp> mrg_op_ptr{Ort::Custom::CreateLiteCustomOp<Merge>("Merge",
//   "CPUExecutionProvider")}; v2_domain.Add(mrg_op_ptr.get()); session_options.Add(v2_domain);
// For the complete example, please search keyword "LiteCustomOpTest" under
// "<cloned_src_dir>/onnxruntime/test/".
...
struct OrtLiteCustomStruct : public OrtLiteCustomOp {
    ...
    OrtLiteCustomStruct(const char* op_name, const char* execution_provider, int start_ver = 1,
                        int end_ver = MAX_CUSTOM_OP_END_VER)
        : OrtLiteCustomOp(op_name, execution_provider, {}, start_ver, end_ver) {
    ...
    void SetCompute(CustomComputeFn<Args...>) {
        ...
        OrtCustomOp::KernelCompute = [](void* op_kernel, OrtKernelContext* context) {
            ...
            auto t = CreateTuple<0, 0, Args...>(context, args, kernel->num_input_,
                                                kernel->num_output_, kernel->ep_);
            std::apply([kernel](Args const&... t_args) { kernel->custom_op_->Compute(t_args...); },
                       t);
        };
    }
...
template <typename... Args>
OrtLiteCustomOp* CreateLiteCustomOp(const char* op_name, const char* execution_provider,
                                    void (*custom_compute_fn)(Args...),
                                    Status (*shape_infer_fn)(ShapeInferContext&) = {},
                                    int start_ver = 1, int end_ver = MAX_CUSTOM_OP_END_VER) {
    using LiteOp = OrtLiteCustomFunc<Args...>;
    return std::make_unique<LiteOp>(op_name, execution_provider, custom_compute_fn, shape_infer_fn,
                                    start_ver, end_ver)
        .release();
}
...
OrtLiteCustomOp* CreateLiteCustomOp(const char* op_name, const char* execution_provider,
                                    ...
                                    int start_ver = 1, int end_ver = MAX_CUSTOM_OP_END_VER) {
    using LiteOp = OrtLiteCustomFunc<Args...>;
    return std::make_unique<LiteOp>(op_name, execution_provider, custom_compute_fn_v2,
                                    shape_infer_fn, start_ver, end_ver)
        .release();
}
