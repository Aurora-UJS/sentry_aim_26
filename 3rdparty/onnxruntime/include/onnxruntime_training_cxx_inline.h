...
inline TrainingSession::TrainingSession(
    ...
    ThrowOnError(GetTrainingApi().CreateTrainingSession(
        env, session_options, checkpoint_state, train_model_path.c_str(),
        eval_model_path.has_value() ? eval_model_path.value().c_str() : nullptr,
        optimizer_model_path.has_value() ? optimizer_model_path.value().c_str() : nullptr, &p_));

    ThrowOnError(GetTrainingApi().TrainingSessionGetTrainingModelOutputCount(
        p_, &training_model_output_count_));

    ThrowOnError(
        GetTrainingApi().TrainingSessionGetEvalModelOutputCount(p_, &eval_model_output_count_));
}
...
inline TrainingSession::TrainingSession(const Env& env, const SessionOptions& session_options,
    ...
    ThrowOnError(GetTrainingApi().CreateTrainingSessionFromBuffer(
        env, session_options, checkpoint_state, train_model_data.data(), train_model_data.size(),
        eval_model_data.data(), eval_model_data.size(), optim_model_data.data(),
        optim_model_data.size(), &p_));
...
inline std::vector<Value> TrainingSession::TrainStep(const std::vector<Value>& input_values) {
    ...
    for (size_t i = 0; i < training_model_output_count_; i++)
    ...
    ThrowOnError(GetTrainingApi().TrainStep(p_, run_options, input_values.size(), ort_input_values,
...
inline std::vector<Value> TrainingSession::EvalStep(const std::vector<Value>& input_values) {
    ...
    for (size_t i = 0; i < eval_model_output_count_; i++)
        output_values.emplace_back(nullptr);
    auto ort_input_values = reinterpret_cast<const OrtValue* const*>(input_values.data());
    auto ort_output_values = reinterpret_cast<OrtValue**>(output_values.data());
    RunOptions run_options;
    ThrowOnError(GetTrainingApi().EvalStep(p_, run_options, input_values.size(), ort_input_values,
...
inline void TrainingSession::RegisterLinearLRScheduler(int64_t warmup_step_count,
                                                       int64_t total_step_count, float initial_lr) {
...
inline std::vector<std::string> TrainingSession::InputNames(const bool training) {
    auto& input_count_function = training
                                     ? GetTrainingApi().TrainingSessionGetTrainingModelInputCount
...
inline std::vector<std::string> TrainingSession::OutputNames(const bool training) {
    auto& output_count_function = training
                                      ? GetTrainingApi().TrainingSessionGetTrainingModelOutputCount
                                      : GetTrainingApi().TrainingSessionGetEvalModelOutputCount;
    auto& output_name_function = training
                                     ? GetTrainingApi().TrainingSessionGetTrainingModelOutputName
...
inline Value TrainingSession::ToBuffer(const bool only_trainable) {
    ...
    AllocatorWithDefaultOptions allocator;
    Value buffer =
        Value::CreateTensor(allocator, buffer_shape.data(), 1U,
...
inline void TrainingSession::FromBuffer(Value& buffer) {
    if (!buffer.IsTensor()) {
        ThrowStatus(Status("Incorrect buffer received. Expected a tensor buffer.",
...
inline CheckpointState CheckpointState::LoadCheckpoint(
...
inline CheckpointState CheckpointState::LoadCheckpointFromBuffer(
    const std::vector<uint8_t>& buffer) {
    OrtCheckpointState* checkpoint_state;
    ThrowOnError(
        GetTrainingApi().LoadCheckpointFromBuffer(buffer.data(), buffer.size(), &checkpoint_state));
    return CheckpointState(checkpoint_state);
}
...
inline void TrainingSession::ExportModelForInferencing(
...
inline void CheckpointState::AddProperty(const std::string& property_name,
                                         const Property& property_value) {
    if (std::holds_alternative<int64_t>(property_value)) {
        int64_t value = std::get<int64_t>(property_value);
        void* value_p = &value;
        ThrowOnError(GetTrainingApi().AddProperty(p_, property_name.c_str(),
                                                  OrtPropertyType::OrtIntProperty, value_p));
    } else if (std::holds_alternative<float>(property_value)) {
        float value = std::get<float>(property_value);
        void* value_p = &value;
        ThrowOnError(GetTrainingApi().AddProperty(p_, property_name.c_str(),
                                                  OrtPropertyType::OrtFloatProperty, value_p));
        ...
        // AddProperty takes a char* and calls PropertyBag::AddProperty which takes a std::string.
        // The data will be copied at that point so buffer can free the local allocation once the
        // call is made.
        ThrowOnError(GetTrainingApi().AddProperty(
            p_, property_name.c_str(), OrtPropertyType::OrtStringProperty, buffer.get()));
    } else {
...
inline Property CheckpointState::GetProperty(const std::string& property_name) {
    ...
    AllocatorWithDefaultOptions allocator;
    ThrowOnError(GetTrainingApi().GetProperty(p_, property_name.c_str(), allocator, &property_type,
                                              &property_value));
    ...
    switch (property_type) {
        ...
        default: {
            ThrowStatus(
                Status("Unknown property type received.", OrtErrorCode::ORT_INVALID_ARGUMENT));
            break;
        }
    }
...
inline void CheckpointState::UpdateParameter(const std::string& parameter_name,
