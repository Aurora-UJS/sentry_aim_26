...
// exceptions on errors and automatically releasing resources in the destructors. The primary
// purpose of C++ API is exception safety so all the resources follow RAII and do not leak memory.
//
...
// e{nullptr};). However, you can't use them until you assign an instance that actually holds an
// underlying object.
//
...
//
// ConstXXXX types are copyable since they do not own the underlying C object, so you can pass them
// to functions as arguments by value or by reference. ConstXXXX types are restricted to const only
// interfaces.
//
// UnownedXXXX are similar to ConstXXXX but also allow non-const interfaces.
//
// The lifetime of the corresponding owning object must eclipse the lifetimes of the
// ConstXXXX/UnownedXXXX types. They exists so you do not have to fallback to C types and the API
// with the usual pitfalls. In general, do not use C API from your C++ code.
...
// Used by custom operator libraries that are not linked to onnxruntime. Sets the global API object,
// which is required by C++ APIs.
//
...
public:
    ...
    /// for two values by or'ing the private bits together and stripping the sign. They are both
    /// zero, and therefore equivalent, if the resulting value is still zero.
    /// </summary>
...
#define ORT_DEFINE_RELEASE(NAME)             \
    inline void OrtRelease(Ort##NAME* ptr) { \
        GetApi().Release##NAME(ptr);         \
    }
...
/** \brief This is a tagging template type. Use it with Base<T> to indicate that the C++ interface
 * object has no ownership of the underlying C object.
 */
template <typename T>
struct Unowned {
    using Type = T;
 ...
 *  b) Each of struct XXXX, XXX instances function as smart pointers to the underlying C API
 * objects. they would release objects owned automatically when going out of scope, they are
 * move-only. c) ConstXXXX and UnownedXXX structs function as non-owning, copyable containers for
 * the above pointers. ConstXXXX allow calling const interfaces only. They give access to objects
 * that are owned by somebody else such as Onnxruntime or instances of XXXX classes. d) serve
 * convenient interfaces that return C++ objects and further enhance exception and type safety so
 * they can be used in C++ code.
 *
 */
...
struct AllocatedFree {
    ...
    void operator()(void* ptr) const {
        if (ptr)
            allocator_->Free(allocator_, ptr);
    }
};
...
struct Status : detail::Base<OrtStatus> {
    ...
    explicit Status(OrtStatus* status) noexcept;      ///< Takes ownership of OrtStatus instance
    ...
    Status(const char* message,
           OrtErrorCode
               code) noexcept;  ///< Creates status instance out of null-terminated string message.
...
struct CustomOpConfigs {
     ...
     */
    CustomOpConfigs& AddConfig(const char* custom_op_name, const char* config_key,
                               const char* config_value);

    /** \brief Returns a flattened map of custom operator configuration entries and their values.
     *
     * The keys has been flattened to include both the custom operator name and the configuration
     * entry key name. For example, a prior call to AddConfig("my_op", "key", "value") corresponds
     * to the flattened key/value pair
     * {"my_op.key", "value"}.
     *
...
struct ConstSessionOptionsImpl : Base<T> {
    ...
    std::string GetConfigEntry(
        const char* config_key) const;                  ///< Wraps OrtApi::GetSessionConfigEntry
...
struct SessionOptionsImpl : ConstSessionOptionsImpl<T> {
    ...
    SessionOptionsImpl& SetDeterministicCompute(
        bool value);  ///< Wraps OrtApi::SetDeterministicCompute
    ...
    SessionOptionsImpl& SetLogSeverityLevel(
        int level);  ///< Wraps OrtApi::SetSessionLogSeverityLevel

    SessionOptionsImpl& Add(
        OrtCustomOpDomain* custom_op_domain);  ///< Wraps OrtApi::AddCustomOpDomain
    ...
    SessionOptionsImpl& AddInitializer(const char* name,
    ...
    SessionOptionsImpl& AddExternalInitializersFromFilesInMemory(
        ...
        const std::vector<size_t>&
            external_initializer_file_lengths);  ///< Wraps
    ...
    /// Wraps OrtApi::SessionOptionsAppendExecutionProvider. Currently supports QNN, SNPE and
    /// XNNPACK.
    ...
    ///< OrtApi::RegisterCustomOpsLibrary_V2. The custom operator configurations are optional. If
    ///< provided, custom operator configs are set via OrtApi::AddSessionConfigEntry.
    SessionOptionsImpl& RegisterCustomOpsLibrary(const ORTCHAR_T* library_name,
                                                 const CustomOpConfigs& custom_op_configs = {});

    SessionOptionsImpl& RegisterCustomOpsUsingFunction(
        const char* function_name);  ///< Wraps OrtApi::RegisterCustomOpsUsingFunction

    ///< Wraps OrtApi::SessionOptionsAppendExecutionProvider_VitisAI
    SessionOptionsImpl& AppendExecutionProvider_VitisAI(
        const std::unordered_map<std::string, std::string>& provider_options = {});
};
}  // namespace detail

using UnownedSessionOptions = detail::SessionOptionsImpl<detail::Unowned<OrtSessionOptions>>;
using ConstSessionOptions =
    detail::ConstSessionOptionsImpl<detail::Unowned<const OrtSessionOptions>>;
...
struct SessionOptions : detail::SessionOptionsImpl<OrtSessionOptions> {
    ...
    explicit SessionOptions(OrtSessionOptions* p)
        : SessionOptionsImpl<OrtSessionOptions>{p} {}  ///< Used for interop with the C API
...
struct ModelMetadata : detail::Base<OrtModelMetadata> {
     ...
     *
     * \param allocator to allocate memory for the copy of the string returned
     * \return a instance std::vector of smart pointers that would deallocate the buffers when out
     * of scope. The OrtAllocator instance must be valid at the point of memory release.
     */
...
struct SessionImpl : ConstSessionImpl<T> {
     ...
     * list of input names
     * \param[in] input_values Array of Value objects of length input_count that is the list of
     * input values
     * \param[in] input_count Number of inputs (the size of the input_names & input_values arrays)
     * \param[in] output_names Array of C style strings of length output_count that is the list of
     * output names
     * \param[in] output_count Number of outputs (the size of the output_names array)
     * \return A std::vector of Value objects that directly maps to the output_names array (eg.
     * output_name[0] is the first entry of the returned vector)
     */
     ...
     *             On calling RunAsync, output_values[i] could either be initialized by a null
     * pointer or a preallocated OrtValue*. Later, on invoking the callback, each output_values[i]
     * of null will be filled with an OrtValue* allocated by onnxruntime. Then, an OrtValue**
     * pointer will be casted from output_values, and pass to the callback. NOTE: it is customer's
     * duty to finally release output_values and each of its member, regardless of whether the
     * member (Ort::Value) is allocated by onnxruntime or preallocated by the customer.
     ...
     */
    void RunAsync(const RunOptions& run_options, const char* const* input_names,
                  const Value* input_values, size_t input_count, const char* const* output_names,
                  Value* output_values, size_t output_count, RunAsyncCallbackFn callback,
                  void* user_data);
    ...
    AllocatedStringPtr EndProfilingAllocated(
        OrtAllocator* allocator);  ///< Wraps OrtApi::SessionEndProfiling
};
...
struct MemoryInfo : detail::MemoryInfoImpl<OrtMemoryInfo> {
    ...
    explicit MemoryInfo(OrtMemoryInfo* p)
        : MemoryInfoImpl<OrtMemoryInfo>{p} {}  ///< Take ownership of a pointer created by C Api
...
struct TensorTypeAndShapeInfoImpl : Base<T> {
    ...
    [[deprecated("use GetShape()")]] void GetDimensions(
        int64_t* values, size_t values_count) const;  ///< Wraps OrtApi::GetDimensions
...
using ConstTensorTypeAndShapeInfo =
    detail::TensorTypeAndShapeInfoImpl<detail::Unowned<const OrtTensorTypeAndShapeInfo>>;
...
using ConstSequenceTypeInfo =
    detail::SequenceTypeInfoImpl<detail::Unowned<const OrtSequenceTypeInfo>>;
...
struct SequenceTypeInfo : detail::SequenceTypeInfoImpl<OrtSequenceTypeInfo> {
    ...
    explicit SequenceTypeInfo(OrtSequenceTypeInfo* p)
        : SequenceTypeInfoImpl<OrtSequenceTypeInfo>{p} {}  ///< Used for interop with the C API
...
struct MapTypeInfo : detail::MapTypeInfoImpl<OrtMapTypeInfo> {
    ...
    explicit MapTypeInfo(OrtMapTypeInfo* p)
        : MapTypeInfoImpl<OrtMapTypeInfo>{p} {}  ///< Used for interop with the C API
...
struct TypeInfoImpl : detail::Base<T> {
    ...
    ConstTensorTypeAndShapeInfo GetTensorTypeAndShapeInfo()
        const;  ///< Wraps OrtApi::CastTypeInfoToTensorInfo
    ConstSequenceTypeInfo GetSequenceTypeInfo()
    ...
    ConstOptionalTypeInfo GetOptionalTypeInfo()
        const;  ///< wraps OrtApi::CastTypeInfoToOptionalTypeInfo
...
struct ConstValueImpl : Base<T> {
    ...
    void GetOpaqueData(const char* domain, const char* type_name,
                       R&) const;  ///< Wraps OrtApi::GetOpaqueValue
    ...
    /// Use of this API is recommended over GetStringTensorElement() that takes void* buffer
    /// pointer.
    /// </summary>
    /// <param name="element_index"></param>
    ...
    /// pointer.</returns>
    template <typename R>
    const R* GetSparseTensorIndicesData(OrtSparseIndicesFormat indices_format,
...
struct ValueImpl : ConstValueImpl<T> {
    ...
    /// </summary>
    /// <param name="s">[in] An array of strings. Each string in this array must be null
    /// terminated.</param> <param name="s_len">[in] Count of strings in s (Must match the size of
    /// \p value's tensor shape)</param>
...
struct Value : detail::ValueImpl<OrtValue> {
    ...
    /** \brief Creates an OrtValue with a tensor using a supplied OrtAllocator. Wraps
     * OrtApi::CreateTensorAsOrtValue. This overload will allocate the buffer for the tensor
     * according to the supplied shape and data type. The allocated buffer will be owned by the
     * returned OrtValue and will be freed when the OrtValue is released. The input data would need
     * to be copied into the allocated buffer. This API is not suitable for strings.
     *
     * \tparam T The numeric datatype. This API is not suitable for strings.
     ...
     *   The allocated buffer will be owned by the returned OrtValue and will be freed when the
     * OrtValue is released. The input data would need to be copied into the allocated buffer. This
     * API is not suitable for strings.
     *
     * \param allocator The allocator to use.
     * \param shape Pointer to the tensor shape dimensions.
     * \param shape_len The number of tensor shape dimensions.
     ...
     *  when the returned OrtValue is released. The caller may release keys and values after the
     * call returns.
     *
     * \param keys an OrtValue containing a tensor with primitive data type keys.
     * \param values an OrtValue that may contain a tensor. Ort currently supports only primitive
     * data type values.
     */
    ...
    static Value CreateOpaque(const char* domain, const char* type_name,
                              const T& value);  ///< Wraps OrtApi::CreateOpaqueValue
...
struct ArenaCfg : detail::Base<OrtArenaCfg> {
     ...
     * See docs/C_API.md for details on what the following parameters mean and how to choose these
     * values
     */
...
/**
 * Macro that logs a message using the provided logger. Throws an exception if
 * OrtApi::Logger_LogMessage fails. Example: ORT_CXX_LOG(logger, ORT_LOGGING_LEVEL_INFO, "Log a
 * message");
 *
...
/**
 * Macro that logs a message using the provided logger. Can be used in noexcept code since errors
 * are silently ignored. Example: ORT_CXX_LOG_NOEXCEPT(logger, ORT_LOGGING_LEVEL_INFO, "Log a
 * message");
 *
 * \param logger The Ort::Logger instance to use. Must be a value or reference.
 * \param message_severity The logging severity level of the message.
 ...
 *               Refer to https://en.cppreference.com/w/cpp/io/c/fprintf for information on valid
 * formats.
...
/**
 * Macro that logs a printf-like formatted message using the provided logger. Can be used in
 * noexcept code since errors are silently ignored. Example: ORT_CXX_LOGF_NOEXCEPT(logger,
 * ORT_LOGGING_LEVEL_INFO, "Log an int: %d", 12);
 *
 * \param logger The Ort::Logger instance to use. Must be a value or reference.
 * \param message_severity The logging severity level of the message.
 * \param format A null-terminated UTF-8 format string forwarded to a printf-like function.
...
/// and to take advantage of a cached logging severity level that can bypass calls to the underlying
/// C API.
/// </summary>
struct Logger {
    /**
    ...
    /**
     * Creates a logger from an ::OrtLogger instance. Caches the logger's current severity level by
     * calling OrtApi::Logger_GetLoggingSeverityLevel. Throws an exception if
     * OrtApi::Logger_GetLoggingSeverityLevel fails.
     *
    ...
    /**
     * Logs the provided message via OrtApi::Logger_LogMessage. Use the ORT_CXX_LOG or
     * ORT_CXX_LOG_NOEXCEPT macros to properly set the source code location and to use the cached
     * severity level to potentially bypass calls to the underlying C API.
     *
     * \param log_severity_level The message's logging severity level.
     * \param file_path The filepath of the file in which the message is logged. Usually the value
     * of ORT_FILE.
     * \param line_number The file line number in which the message is logged. Usually the value of
     * __LINE__.
     * \param func_name The name of the function in which the message is logged. Usually the value
     * of __FUNCTION__.
     * \param message The message to log.
     * \return A Ort::Status value to indicate error or success.
     */
    Status LogMessage(OrtLoggingLevel log_severity_level, const ORTCHAR_T* file_path,
                      int line_number, const char* func_name, const char* message) const noexcept;

    /**
     * Logs a printf-like formatted message via OrtApi::Logger_LogMessage. Use the ORT_CXX_LOGF or
     * ORT_CXX_LOGF_NOEXCEPT macros to properly set the source code location and to use the cached
     * severity level to potentially bypass calls to the underlying C API. Returns an error status
     ...
     * of __FUNCTION__.
     * \param format A null-terminated UTF-8 format string forwarded to a printf-like function.
     *               Refer to https://en.cppreference.com/w/cpp/io/c/fprintf for information on
     * valid formats.
     * \param args Zero or more variadic arguments referenced by the format string.
     * \return A Ort::Status value to indicate error or success.
     */
    template <typename... Args>
    Status LogFormattedMessage(OrtLoggingLevel log_severity_level, const ORTCHAR_T* file_path,
...
struct KernelContext {
    ...
    void ParallelFor(void (*fn)(void*, size_t), size_t total, size_t num_batch,
                     void* usr_data) const;
...
struct Op : detail::Base<OrtOp> {
    ...
    void Invoke(const OrtKernelContext* context, const Value* input_values, size_t input_count,
                Value* output_values, size_t output_count);

    // For easier refactoring
    void Invoke(const OrtKernelContext* context, const OrtValue* const* input_values,
                size_t input_count, OrtValue* const* output_values, size_t output_count);
};

/// <summary>
/// Provide access to per-node attributes and input shapes, so one could compute and set output
/// shapes.
/// </summary>
...
#pragma warning(pop)
        ...
        OrtCustomOp::GetVariadicInputHomogeneity = [](const OrtCustomOp* this_) {
            return static_cast<int>(static_cast<const TOp*>(this_)->GetVariadicInputHomogeneity());
        };
        ...
        OrtCustomOp::GetVariadicOutputHomogeneity = [](const OrtCustomOp* this_) {
            return static_cast<int>(static_cast<const TOp*>(this_)->GetVariadicOutputHomogeneity());
        };
        ...
        if (WithStatus) {
#endif
            OrtCustomOp::CreateKernelV2 = [](const OrtCustomOp* this_, const OrtApi* api,
                                             const OrtKernelInfo* info,
                                             void** op_kernel) -> OrtStatusPtr {
                return static_cast<const TOp*>(this_)->CreateKernelV2(*api, info, op_kernel);
            };
            OrtCustomOp::KernelComputeV2 = [](void* op_kernel,
                                              OrtKernelContext* context) -> OrtStatusPtr {
    ...
    OrtMemType GetInputMemoryType(size_t /*index*/) const { return OrtMemTypeDefault; }

    // Default implementation of GetVariadicInputMinArity() returns 1 to specify that a variadic
    // input should expect at least 1 argument.
    int GetVariadicInputMinArity() const { return 1; }

    // Default implementation of GetVariadicInputHomegeneity() returns true to specify that all
    // arguments to a variadic input should be of the same type.
    bool GetVariadicInputHomogeneity() const { return true; }

    // Default implementation of GetVariadicOutputMinArity() returns 1 to specify that a variadic
    // output should produce at least 1 output value.
    int GetVariadicOutputMinArity() const { return 1; }

    // Default implementation of GetVariadicOutputHomegeneity() returns true to specify that all
    // output values produced by a variadic output should be of the same type.
    bool GetVariadicOutputHomogeneity() const { return true; }
    ...
    std::vector<std::string> GetSessionConfigKeys() const { return std::vector<std::string>{}; }

    template <typename C>
    decltype(&C::InferOutputShape) SetShapeInferFn(decltype(&C::InferOutputShape)) {
        OrtCustomOp::InferOutputShapeFn = [](const OrtCustomOp*,
...
protected:
    ...
    void GetSessionConfigs(std::unordered_map<std::string, std::string>& out,
