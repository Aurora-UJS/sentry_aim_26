...
#define ORT_DECLARE_TRAINING_RELEASE(NAME) void OrtRelease(Ort##NAME* ptr);
...
/// <summary>
/// This function returns the C training api struct with the pointers to the ort training C
/// functions. If using C++, please use the class instances instead of invoking the C functions
/// directly.
/// </summary>
/// <returns>OrtTrainingApi struct with ort training C function pointers.</returns>
inline const OrtTrainingApi& GetTrainingApi() {
    return *GetApi().GetTrainingApi(ORT_API_VERSION);
}
...
#define ORT_DEFINE_TRAINING_RELEASE(NAME)    \
    inline void OrtRelease(Ort##NAME* ptr) { \
        GetTrainingApi().Release##NAME(ptr); \
    }
 ...
 *
 * This class holds the entire training session state that includes model parameters, their
 * gradients, optimizer parameters, and user properties. The Ort::TrainingSession leverages the
 * Ort::CheckpointState by accessing and updating the contained training state.
 * \note Note that the training session created with a checkpoint state uses this state to store the
 * entire training state (including model parameters, its gradients, the optimizer states and the
 * properties). The Ort::TrainingSession does not hold a copy of the Ort::CheckpointState and as a
 * result, it is required that the checkpoint state outlive the lifetime of the training session.
 * \note Note that the checkpoint state can be either the complete checkpoint state or the nominal
 * checkpoint state depending on the version provided while loading the checkpoint.
 *
 */
...
public:
     ...
     * state and return an instance of Ort::CheckpointState. This checkpoint state can then be used
     * to create the training session by instantiating Ort::TrainingSession. By doing so, the
     * training session will resume training from the given checkpoint state.
     *
     ...
     * This checkpoint can later be loaded by invoking Ort::CheckpointState::LoadCheckpoint to
     * resume training from this snapshot of the state.
     *
     ...
     * parameter name. The parameter is copied over to the provided OrtValue. The training session
     * must be already created with the checkpoint state that contains the parameter being
...
/** \brief Trainer class that provides training, evaluation and optimizer methods for training an
 * ONNX models.
 *
...
public:
     ...
     *
     * This constructor instantiates the training session based on the env and session options
     * provided that can begin or resume training from a given checkpoint state for the given onnx
     * models. The checkpoint state represents the parameters of the training session which will be
     * moved to the device specified by the user through the session options (if necessary).
     *
     * \param[in] env Env to be used for the training session.
     * \param[in] session_options SessionOptions that the user can customize for this training
     * session.
     ...
     */
    TrainingSession(
        const Env& env, const SessionOptions& session_options, CheckpointState& checkpoint_state,
     ...
     * \param[in] optim_model_data Buffer containing optimizer model (used for performing
     * weight/parameter update).
     *
     */
    TrainingSession(const Env& env, const SessionOptions& session_options,
                    CheckpointState& checkpoint_state, const std::vector<uint8_t>& train_model_data,
     ...
     * Ort::TrainingSession::TrainStep is equivalent of running forward propagation and backward
     * propagation in a single step. The gradients computed are stored inside the training session
     * state so they can be later consumed by the Ort::TrainingSession::OptimizerStep function. The
     * gradients can be lazily reset by invoking the Ort::TrainingSession::LazyResetGrad function.
     *
     * \param[in] input_values The user inputs to the training model.
     * \return A std::vector of Ort::Value objects that represents the output of the forward pass of
     * the training model.
     *
     *
     */
     ...
     * new gradients are computed on the next invocation of the next
     * Ort::TrainingSession::TrainStep.
     *
     */
    void LazyResetGrad();
     ...
     * Ort::TrainingSession::RegisterLinearLRScheduler.
     *
     ...
     * inferencing with Ort::Session.
     * \note Note that the function re-loads the eval model from the path provided to
     * Ort::TrainingSession and expects that this path still be valid.
     *
     ...
     *
     * This function returns the names of inputs of the training or eval model that can be
     * associated with the Ort::Value(s) provided to the Ort::TrainingSession::TrainStep or
     * Ort::TrainingSession::EvalStep function.
     *
     * \param[in] training Whether the training model input names are requested or eval model input
     * names.
     ...
     *
     * This function returns the names of outputs of the training or eval model that can be
     * associated with the Ort::Value(s) returned by the Ort::TrainingSession::TrainStep or
     * Ort::TrainingSession::EvalStep function.
     *
     * \param[in] training Whether the training model output names are requested or eval model
     * output names.
     ...
     *
     * \param[in] only_trainable Whether to only copy trainable parameters or to copy all
     * parameters.
     ...
     *
     * In case the training session was created with a nominal checkpoint, invoking this function is
     * required to load the updated parameters onto the checkpoint to complete it.
     *
