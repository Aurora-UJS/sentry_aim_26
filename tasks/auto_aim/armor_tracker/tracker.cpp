...
TrackerConfig TrackerConfig::fromYaml(const std::string& yaml_path) {
    ...
    try {
        ...
        if (tracker_node["measurement_position_noise"]) {
            config.measurement_position_noise = tracker_node["measurement_position_noise"].as<double>();
        }
...
public:
    ...
    EKFFilter(const TrackerConfig& config, const State& initial_state) 
        ...
        auto update_Q = [this]() -> Eigen::Matrix<double, N_X, N_X> {
            ...
            Q.diagonal() << 
                config_.position_noise, config_.position_noise, config_.position_noise,
        ...
        auto update_R = [this](const Eigen::Vector4d&) -> Eigen::Matrix<double, N_Z, N_Z> {
            ...
            R.diagonal() << 
                config_.measurement_position_noise,
                config_.measurement_position_noise,
                config_.measurement_position_noise,
                config_.measurement_yaw_noise;
            return R;
        };
        ...
        ekf_->setResidualFunc([](const Eigen::Vector4d& z_pred, const Eigen::Vector4d& z_meas) -> Eigen::Vector4d {
    ...
    void predict(double dt) override {
        ...
        while (x(6) > M_PI) x(6) -= 2.0 * M_PI;
        while (x(6) < -M_PI) x(6) += 2.0 * M_PI;
        ekf_->setStatePost(x);
    }
    
    void update(const Eigen::Vector4d& measurement) override {
...
ArmorTracker::ArmorTracker(const TrackerConfig& config, const Armor& initial_armor)
    : config_(config), armor_number_(initial_armor.number), 
...
void ArmorTracker::initFilter(const Armor& initial_armor) {
    ...
    initial_state << 
        initial_armor.pos.x(), initial_armor.pos.y(), initial_armor.pos.z(),
        0.0, 0.0, 0.0,  // 速度初始化为0
        yaw, 0.0;       // yaw和角速度
...
Armor ArmorTracker::getPredictedArmor(double dt) const {
    ...
    if (dt > 0) {
        ...
        // 归一化yaw
        while (state(6) > M_PI) state(6) -= 2.0 * M_PI;
        while (state(6) < -M_PI) state(6) += 2.0 * M_PI;
    }
...
double ArmorTracker::computeMatchDistance(const Armor& armor) const {
    ...
    while (yaw_diff > M_PI) yaw_diff = 2.0 * M_PI - yaw_diff;
...
TrackerManager::TrackerManager(const TrackerConfig& config)
    : config_(config) {
...
void TrackerManager::update(const std::vector<Armor>& armors, 
    ...
    for (int id : matched_tracker_ids) {
        if (id >= 0) tracker_matched[id] = true;
    }
...
void TrackerManager::matchArmorsToTrackers(const std::vector<Armor>& armors,
    ...
    for (size_t iter = 0; iter < std::min(trackers_.size(), armors.size()); ++iter) {
        ...
        for (size_t i = 0; i < trackers_.size(); ++i) {
            if (tracker_used[i]) continue;
            for (size_t j = 0; j < armors.size(); ++j) {
                if (armor_used[j]) continue;
        ...
        // 检查距离阈值
        if (best_tracker >= 0 && best_armor >= 0 && 
            min_dist < config_.max_match_distance) {
