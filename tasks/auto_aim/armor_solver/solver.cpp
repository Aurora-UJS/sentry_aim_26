...
PnpSolver::PnpSolver(const std::string& yaml_path) {
    ...
    utils::logger()->debug("[PnpSolver] 小装甲板尺寸: {}x{} m", SMALL_ARMOR_WIDTH, SMALL_ARMOR_HEIGHT);
    utils::logger()->debug("[PnpSolver] 大装甲板尺寸: {}x{} m", LARGE_ARMOR_WIDTH, LARGE_ARMOR_HEIGHT);
}
...
bool PnpSolver::solve(const ArmorObject& obj, Armor& result) {
    ...
    // 3. 选择对应的 3D 模型点
    const auto& object_points = (obj.type == ArmorType::LARGE) ? 
                                 large_armor_points_ : small_armor_points_;

    // 4. 执行 PnP 解算
    cv::Mat rvec, tvec;
    // 使用 SOLVEPNP_IPPE: 适用于平面四点，效果好、速度快、稳定性强
    bool success = cv::solvePnP(object_points, obj.pts, camera_matrix_, dist_coeffs_, 
    ...
    // 8. 记录详细信息
    utils::logger()->debug("[PnpSolver] 解算成功 - 类型: {}, 位置: ({:.3f}, {:.3f}, {:.3f}) m, 距离: {:.3f} m",
