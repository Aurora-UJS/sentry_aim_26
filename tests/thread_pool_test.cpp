...
TEST_CASE("OrderedFrameProcessing", "[ThreadPool]") {
  ...
  for (int id : ids) {
    Frame frame{id};
    futures.push_back(
        pool.submit_frame(frame, [&mtx, &processed_ids](const Frame &f) {
...
TEST_CASE("DropOutdatedFrames", "[ThreadPool]") {
  auto drop_pred = [](const Frame &f) {
    ...
    return std::chrono::duration_cast<std::chrono::milliseconds>(now - f.t)
               .count() > 500;
  };
...
TEST_CASE("ConcurrentFrameSubmission", "[ThreadPool]") {
  ...
  for (int t = 0; t < 4; ++t) {
    submitters.emplace_back([&pool, t, num_frames, &mtx, &processed_ids,
                             &futures, &futures_mtx]() {
...
TEST_CASE("QueueFull", "[ThreadPool]") {
  ...
  Frame frame{3};
  REQUIRE_THROWS_AS(
      small_pool.submit_frame(
          frame, [](const Frame &f) { return std::vector<int>{f.id}; }),
      std::runtime_error);
...
TEST_CASE("Performance", "[ThreadPool]") {
  ...
  auto duration =
      std::chrono::duration_cast<std::chrono::milliseconds>(end - start)
          .count();
  std::cout << "Processed " << num_tasks << " tasks in " << duration << " ms"
            << std::endl;
}