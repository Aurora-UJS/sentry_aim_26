...
void drawVisualization(cv::Mat& img, 
    ...
    for (size_t i = 0; i < detections.size(); ++i) {
        ...
        cv::putText(img, ss.str(), obj.center + cv::Point2f(10, -10),
    ...
    for (size_t i = 0; i < solved_armors.size() && i < detections.size(); ++i) {
        ...
        cv::putText(img, ss.str(), obj.center + cv::Point2f(10, 28),
    ...
    if (tracked_target.has_value()) {
        ...
        // 在图像顶部显示跟踪信息
        cv::rectangle(img, cv::Point(10, 10), cv::Point(500, 120), 
                     cv::Scalar(0, 0, 0), -1);
        cv::rectangle(img, cv::Point(10, 10), cv::Point(500, 120), 
                     cv::Scalar(0, 255, 0), 2);
        ...
        cv::putText(img, ss.str(), cv::Point(20, y_offset),
                   cv::FONT_HERSHEY_SIMPLEX, 0.6, cv::Scalar(0, 255, 0), 2);
        y_offset += 25;
        
        ss.str("");
        ss << "Predicted Pos: (" << std::fixed << std::setprecision(2)
           << target.pos.x() << ", " << target.pos.y() << ", " << target.pos.z() << ")m";
        cv::putText(img, ss.str(), cv::Point(20, y_offset),
...
int main(int argc, char** argv) {
    ...
    // 参数检查
    if (argc < 5) {
        utils::logger()->error("Usage: {} <video_path> <model_path> <camera_config> <tracker_config>", argv[0]);
        utils::logger()->info("Example: {} video/demo.avi model/armor.onnx config/camera_info_demo.yaml config/tracker_config.yaml", 
                            argv[0]);
        return -1;
    }
    ...
    while (true) {
        ...
        if (!paused) {
            ...
            auto detect_time = std::chrono::duration_cast<std::chrono::milliseconds>(t2 - t1).count();
            ...
            auto solve_time = std::chrono::duration_cast<std::chrono::milliseconds>(t2 - t1).count();
            ...
            t2 = std::chrono::high_resolution_clock::now();
            auto track_time = std::chrono::duration_cast<std::chrono::milliseconds>(t2 - t1).count();
            ...
            ss << "Frame: " << frame_count << " | Detected: " << detections.size() 
               << " | Solved: " << solve_success << " | Tracked: " << tracker_manager.getTrackers().size();
    ...
    utils::logger()->info("Tracking rate: {:.1f}%", 
                         100.0 * tracking_count / frame_count);
