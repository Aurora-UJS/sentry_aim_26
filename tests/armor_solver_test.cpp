...
const Eigen::Matrix3d R_camera2gimbal =
    (Eigen::Matrix3d() << -0.0083195760046954614, 0.010498791137270739, 0.99991027599468041,
     -0.99960756138647755, -0.026835747568381807, -0.0080352891314148939, 0.026748978935305992,
     -0.99958472279097077, 0.010717933047771133)
        .finished();

const Eigen::Vector3d t_camera2gimbal(0.094969301833534511, 0.095006290298006682,
                                      0.050987066291756609);
...
// 绘制坐标系辅助函数
void drawAxis(cv::Mat& img, const cv::Mat& rvec, const cv::Mat& tvec, const cv::Mat& camera_matrix,
              const cv::Mat& dist_coeffs, float length = 0.1f) {
    // 定义坐标系的3个轴（单位：米）
...
void drawArmorInfo(cv::Mat& img, const armor::Armor& armor, const cv::Point2f& center,
    ...
    cv::putText(img, ss.str(), center + cv::Point2f(10, line_offset), cv::FONT_HERSHEY_SIMPLEX, 0.6,
    ...
    cv::putText(img, ss.str(), center + cv::Point2f(10, line_offset), cv::FONT_HERSHEY_SIMPLEX, 0.5,
    ...
    // 第3行：位置
    ss << "Pos: (" << std::fixed << std::setprecision(2) << armor.pos.x() << "," << armor.pos.y()
       << "," << armor.pos.z() << ")";
    cv::putText(img, ss.str(), center + cv::Point2f(10, line_offset), cv::FONT_HERSHEY_SIMPLEX, 0.5,
                cv::Scalar(255, 255, 0), 2);
    line_offset += 20;
    ...
    cv::putText(img, ss.str(), center + cv::Point2f(10, line_offset), cv::FONT_HERSHEY_SIMPLEX, 0.4,
    ...
    if (gyro_quat != nullptr) {
        ...
        cv::putText(img, ss.str(), center + cv::Point2f(10, line_offset), cv::FONT_HERSHEY_SIMPLEX,
                    0.45, cv::Scalar(150, 150, 150), 1);
        line_offset += 16;
    }
}
...
int main(int argc, char** argv) {
    ...
    if (argc < 2) {
        utils::logger()->error("[Test] 用法: {} <视频路径> [陀螺仪数据路径] [相机配置路径]",
                               argv[0]);
        utils::logger()->info(
            "[Test] 示例: {} video/demo.avi video/demo.txt config/camera_info.yaml", argv[0]);
        return -1;
    }
    ...
    while (true) {
        ...
        for (size_t i = 0; i < armor_objects.size() && i < armors.size(); i++) {
            ...
            cv::Mat rot_mat = (cv::Mat_<double>(3, 3) << rot(0, 0), rot(0, 1), rot(0, 2), rot(1, 0),
                               rot(1, 1), rot(1, 2), rot(2, 0), rot(2, 1), rot(2, 2));
            ...
            // 绘制信息文本（包含陀螺仪对比）
            const Eigen::Quaterniond* gyro_quat =
                has_gyro_data && current_gyro ? &current_gyro->quat : nullptr;
            drawArmorInfo(display, armor, obj.center, gyro_quat);
        }

        // 绘制统计信息
        std::stringstream info;
        info << "Frame: " << frame_count << " | Detected: " << armor_objects.size()
             << " | Solved: " << armors.size();
        if (has_gyro_data && current_gyro) {
            info << " | Gyro: (" << std::fixed << std::setprecision(3) << current_gyro->quat.w()
                 << "," << current_gyro->quat.x() << "," << current_gyro->quat.y() << ","
                 << current_gyro->quat.z() << ")";
        }
        cv::putText(display, info.str(), cv::Point(10, 30), cv::FONT_HERSHEY_SIMPLEX, 0.7,
                    cv::Scalar(0, 255, 0), 2);

        info.str("");
        info << "Detect: " << detect_duration << "ms | Solve: " << solve_duration << "ms";
        cv::putText(display, info.str(), cv::Point(10, 60), cv::FONT_HERSHEY_SIMPLEX, 0.7,
                    cv::Scalar(0, 255, 0), 2);

        if (paused) {
            cv::putText(display, "PAUSED", cv::Point(10, 90), cv::FONT_HERSHEY_SIMPLEX, 0.8,
                        cv::Scalar(0, 0, 255), 2);
        }
