...
// 测试用的相机参数
const cv::Mat TEST_CAMERA_MATRIX =
    (cv::Mat_<double>(3, 3) << 1807.12121, 0, 711.11997, 0, 1806.46896, 562.49495, 0, 0, 1);
...
TEST_CASE("PnP求解 - 小装甲板", "[solver]") {
    ...
    SECTION("正面小装甲板 - 距离1米") {
        // 设置真实位姿：正前方1米，带轻微旋转以规避平面退化情况
        cv::Mat true_rvec = (cv::Mat_<double>(3, 1) << 0, 0.05, 0);
        cv::Mat true_tvec = (cv::Mat_<double>(3, 1) << 0, 0, 1.0);  // Z方向1米

        // 生成小装甲板的3D点
        std::vector<cv::Point3f> object_points = {{-static_cast<float>(SMALL_ARMOR_WIDTH / 2),
                                                   -static_cast<float>(SMALL_ARMOR_HEIGHT / 2), 0},
                                                  {-static_cast<float>(SMALL_ARMOR_WIDTH / 2),
                                                   static_cast<float>(SMALL_ARMOR_HEIGHT / 2), 0},
                                                  {static_cast<float>(SMALL_ARMOR_WIDTH / 2),
                                                   static_cast<float>(SMALL_ARMOR_HEIGHT / 2), 0},
                                                  {static_cast<float>(SMALL_ARMOR_WIDTH / 2),
        ...
        auto image_points = generateSyntheticImagePoints(object_points, true_rvec, true_tvec,
                                                         TEST_CAMERA_MATRIX, TEST_DIST_COEFFS);
        ...
        // 验证位置（允许小误差）
        CHECK(std::isfinite(result.pos.x()));
        CHECK(std::isfinite(result.pos.y()));
        CHECK(std::isfinite(result.pos.z()));
        CHECK_THAT(result.pos.x(), WithinAbs(0.0, 0.01));
        CHECK_THAT(result.pos.y(), WithinAbs(0.0, 0.01));
        CHECK_THAT(result.pos.z(), WithinAbs(1.0, 0.01));

        // 验证距离
        double distance = result.pos.norm();
        CHECK_THAT(distance, WithinAbs(1.0, 0.01));
    }
...
TEST_CASE("PnP求解 - 大装甲板", "[solver]") {
    ...
    SECTION("正面大装甲板 - 距离3米") {
        ...
        std::vector<cv::Point3f> object_points = {{-static_cast<float>(LARGE_ARMOR_WIDTH / 2),
                                                   -static_cast<float>(LARGE_ARMOR_HEIGHT / 2), 0},
                                                  {-static_cast<float>(LARGE_ARMOR_WIDTH / 2),
                                                   static_cast<float>(LARGE_ARMOR_HEIGHT / 2), 0},
                                                  {static_cast<float>(LARGE_ARMOR_WIDTH / 2),
                                                   static_cast<float>(LARGE_ARMOR_HEIGHT / 2), 0},
                                                  {static_cast<float>(LARGE_ARMOR_WIDTH / 2),
...
TEST_CASE("PnP求解 - 姿态测试", "[solver]") {
    ...
    SECTION("装甲板有旋转") {
        ...
        // 验证四元数是单位四元数
        double quat_norm =
            std::sqrt(result.ori.w() * result.ori.w() + result.ori.x() * result.ori.x() +
                      result.ori.y() * result.ori.y() + result.ori.z() * result.ori.z());
