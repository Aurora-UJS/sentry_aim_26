...
struct YpdCoord {
    ...
    YpdCoord(const double& y, const double& p, const double& d)
        : yaw(y), pitch(p), dis(d) {}  // NOLINT(bugprone-easily-swappable-parameters)
    double yaw = 0.;
    double pitch = 0.;
    double dis = 0.;

    math::YpdCoord operator+(const math::YpdCoord& ypd) const {
...
struct Line {
    Line(const double& a, const double& b, const double& c)
        : a(a), b(b), c(c) {}  // NOLINT(bugprone-easily-swappable-parameters)
    double a, b, c;
};
...
template <typename T>
T clamp_default(const T& x, const T& lower, const T& upper,
                const T& default_value) {  // NOLINT(bugprone-easily-swappable-parameters)
    if (x < lower || x > upper) {
...
inline cv::Rect2f get_box(const cv::Point2f pts[4]) {  // NOLINT(cppcoreguidelines-avoid-c-arrays)
    cv::Rect2f box;
...
inline float get_box_iou(const cv::Point2f pts1[4],
                         const cv::Point2f pts2[4]) {  // NOLINT(cppcoreguidelines-avoid-c-arrays)
    cv::Rect2f box1 = math::get_box(pts1);
    cv::Rect2f box2 = math::get_box(pts2);
    if ((box1 & box2).area() == 0.F) {
        return 0.f;
    }
...
inline double reduced(const double& x,
                      const double& range) {  // NOLINT(bugprone-easily-swappable-parameters)
...
inline double get_weighted_angle(
    const double& theta1, const double& w1, const double& theta2,
    const double& w2) {  // NOLINT(bugprone-easily-swappable-parameters)
    double rotate_angle = reduced_angle(theta2 - theta1);
    return math::reduced_angle(theta1 + rotate_angle * (w2 / (w1 + w2)));
}

inline double get_closest(const double& cur, const double& tar,
                          const double& period) {  // NOLINT(bugprone-easily-swappable-parameters)
    double reduced = math::reduced(cur, period);
    std::array<double, 3> possibles = {reduced - period, reduced, reduced + period};
...
inline Eigen::Vector2d get_intersection(const Eigen::Vector2d& pt_a, const Eigen::Vector2d& norm_a,
                                        const Eigen::Vector2d& pt_b,
                                        const Eigen::Vector2d& norm_b) {
    return math::get_intersection(math::pt_and_norm_to_line(pt_a, norm_a),
...
template <typename T>
void ceres_xyz_to_ypd(const T xyz[3], T ypd[3]) {  // NOLINT(cppcoreguidelines-avoid-c-arrays)
...
auto distort_points(const std::vector<cv::Point2f>& src, std::vector<cv::Point2f>& dst,
                    const cv::Mat& camera_mat, const cv::Mat& distortion_mat) -> void;
...
public:
    ...
    std::pair<ValueT, ValueT> find(ValueT left, ValueT right, Func&& cost_function,
