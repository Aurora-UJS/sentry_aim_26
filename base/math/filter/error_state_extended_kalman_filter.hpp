...
/**
 * @brief Error-State Extended Kalman Filter with covariance accessors and online NIS/NEES anomaly
 * detection.
 *
...
public:
    ...
    explicit ErrorStateEKF(const PredicFunc& f, const MeasureFunc& h, const UpdateQFunc& u_q,
                           const UpdateRFunc& u_r, const MatrixXX& P0) noexcept
        : f(f), h(h), update_Q(u_q), update_R(u_r), P_delta(P0), P_delta_pri(P0) {
        F.setZero();
        H.setZero();
    ...
    void setInjectFunc(const InjectFunc& inject_func) { inject_state = inject_func; }
     ...
     *             若未设置，则使用默认实现（元素相减）。
     */
    void setResidualFunc(const ResidualFunc& func) { residual_func_ = func; }

    /**
     * @brief Get prior error-state covariance (after predict, before update).
     * @return Reference to P_delta_pri.
     */
    const MatrixXX& getPriorCovariance() const noexcept { return P_delta_pri; }

    /**
     * @brief Get posterior error-state covariance (after update).
     * @return Reference to P_delta.
     */
    const MatrixXX& getPosteriorCovariance() const noexcept { return P_delta; }

    /**
     * @brief Get current nominal state.
     */
    const MatrixX1& getState() const noexcept { return x_nominal; }

    /**
     * @brief Get the L2 norm of the last measurement residual (after update).
     */
    double getResidualNorm() const noexcept { return last_residual_.norm(); }
    void setUpdateQ(const UpdateQFunc& u_q) { this->update_Q = u_q; }

    void setUpdateR(const UpdateRFunc& u_r) { this->update_R = u_r; }
    // ----------------- anomaly detection API -----------------
    void setNisThreshold(double t) { nis_threshold_ = t; }
    void clearNisThreshold() { nis_threshold_.reset(); }
    void setNeesThreshold(double t) { nees_threshold_ = t; }
    void clearNeesThreshold() { nees_threshold_.reset(); }
    ...
    void setRecentFailRateThreshold(double rate) { recent_fail_rate_threshold_ = rate; }

    double lastNis() const noexcept { return last_nis_; }
    double lastNees() const noexcept { return last_nees_; }
    int totalChecks() const noexcept { return total_count_; }
    int nisFailureCount() const noexcept { return nis_count_; }
    int neesFailureCount() const noexcept { return nees_count_; }
