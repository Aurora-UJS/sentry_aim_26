...
/**
 * @brief Unscented Kalman Filter (UKF) implementation with optional Gauss-Newton style iterative
 * update.
 *
 * This UKF uses the Unscented Transform to handle nonlinear process and measurement models.
 * An optional iterative update can refine the posterior state estimate via multiple measurement
 * updates. Supports handling of angular dimensions with user-provided residual function (e.g.
...
public:
    ...
    explicit UnscentedKalmanFilter(const PredictFunc& f, const MeasureFunc& h,
                                   ...
                                   const MatrixXX& P0, double alpha = 1e-3, double beta = 2.0,
     ...
     */
    void setIterationNum(int num) { iteration_num_ = std::max(1, num); }
