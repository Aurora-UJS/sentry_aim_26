...
math::YpdCoord get_ypd_v(
    const Eigen::Vector3d& xyz,
    const Eigen::Vector3d& xyz_v) {  // NOLINT(bugprone-easily-swappable-parameters)
    double x = xyz(0, 0);
    double y = xyz(1, 0);
    double z = xyz(2, 0);
    double x_dot = xyz_v(0, 0);
    double y_dot = xyz_v(1, 0);
    double z_dot = xyz_v(2, 0);
...
math::YpdCoord camera_get_ypd_v(
    const Eigen::Vector3d& xyz,
    const Eigen::Vector3d& xyz_v) {  // NOLINT(bugprone-easily-swappable-parameters)
    double x = xyz(0, 0);
    double y = xyz(1, 0);
